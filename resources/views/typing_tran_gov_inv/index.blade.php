          ...
          <table id="tt-table" class="table table-striped table-hover align-middle">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td>
                    @if($customerId)
                        <a href="{{ route('finance.statementOfAccount', $customerId) }}" target="_blank" class="fw-bold text-decoration-none text-dark">
                            {{ $customerName }} <i class="fas fa-external-link-alt ms-1 text-muted small"></i>
                        </a>
                    @else
                        {{ $customerName }}
                    @endif
                ...
                <td>{{ number_format($item->amount_of_invoice, 2) }}</td>
                <td>{{ number_format($remainingAmount, 2) }}</td>
                <td><span class="badge {{ $statusBadge }}">{{ $paymentStatus }}</span></td>
                <td>{{ $item->created_at->format('Y-m-d') }}</td>
                <td class="text-center" style="white-space: nowrap;">
                  @if($item->journal)
                    <a href="{{ route('finance.journals.show', $item->journal->id) }}" target="_blank" class="btn btn-secondary btn-sm me-1" title="View Journal Entries">
          ...
          <div class="invoice-wrapper" style="padding: 25px; font-family: Arial, sans-serif; font-size: 12px; color: #333; background: #fff;">
            ...
            <div style="border: 3px solid #333; border-radius: 15px; padding: 15px 20px; margin-bottom: 20px; display: flex; align-items: center; justify-content: space-between;">
              <div style="flex-shrink: 0; margin-right: 20px;">
                <img src="{{ asset('assets/img/' . $logoFileName) }}" alt="Company Logo" style="max-height: 90px; max-width: 150px;" id="inv_company_logo">
              </div>
