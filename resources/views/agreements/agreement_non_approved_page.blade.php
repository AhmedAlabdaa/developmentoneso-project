...
<style>
  ...
  .agreement-info small{font-size:12px;color:#343a40}
    ...
    <div class="card">
      <div class="card-body">
        ...
        <div class="action-buttons">
          <a href="{{ route('agreements.index') }}" class="btn btn-primary btn-sm">
            <i class="fas fa-arrow-left"></i> Back to Agreements
          </a>
          @php $isApproved = ($agreement->marked === 'Yes'); @endphp
          <button id="btnToggleApproval" class="btn btn-sm {{ $isApproved ? 'btn-outline-danger' : 'btn-success' }}" data-marked="{{ $isApproved ? 'Yes' : 'No' }}" type="button">
            @if($isApproved)
        ...
        <div class="agreement-info text-start">
          <small>Agreement Reference No:</small>
          <strong>{{ $agreement->reference_no }}</strong>
        </div>
        ...
        <div class="table-responsive mt-3">
          <table class="table table-striped table-hover">
            ...
            <tbody>
              <tr>
                <td>{{ strtoupper($agreement->candidate_name) }}</td>
                <td>{{ strtoupper(optional($agreement->client)->first_name . ' ' . optional($agreement->client)->last_name) }}</td>
        ...
        <div id="statusAlert" class="alert {{ $isApproved ? 'alert-success' : 'alert-warning' }} text-center mt-4">
          @if($isApproved)
            <i class="fas fa-check-circle"></i>
            This agreement is approved by management.
          @else
            <i class="fas fa-exclamation-triangle"></i>
            This agreement is not yet verified by management.<br>
...
<script>
  (function () {
    ...
    const refNo = @json($agreement->reference_no);
    ...
    function renderButton(marked){
      ...
      if(marked==='Yes'){
        ...
        statusAlert.innerHTML=`<i class="fas fa-check-circle"></i> This agreement is approved by management.`;
      }else{
        ...
        statusAlert.innerHTML=`<i class="fas fa-exclamation-triangle"></i> This agreement is not yet verified by management.<br><i class="fas fa-info-circle"></i> Please request management approval before performing any actions.`;
      }
    }
    ...
    btn.addEventListener('click',function(){
      ...
      setLoading(true);
      fetch(`{{ route('agreements.update-marked') }}`,{
        method:'POST',
