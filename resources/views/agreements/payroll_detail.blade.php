...
<head>
  ...
  <title>Salary Sheet | {{ $payroll->reference_no }}</title>
  <link
  ...
  <div class="container my-4">
    ...
    <div class="salary-container">
      <div class="row align-items-center sheet-header">
        ...
        <div class="col-sm-8 text-end">
          ...
          <div class="mt-2">
            ...
            <span class="info-value">{{ $payroll->reference_no }}</span><br />
          ...
          <tbody>
            ...
            @forelse($details as $detail)
              ...
              <tr>
                ...
                <td>
                  <a
                    href="{{ route('agreements.show', $detail->agreement_reference_no) }}"
                ...
                <td>
                  <a
                    ...
                    class="text-decoration-none text-dark fw-medium"
                  >
                    {{ $detail->agreement_reference_no }}
                  </a>
                </td>
                <td class="text-end">{{ number_format($detail->salary_amount, 2) }}</td>
                <td class="text-end">{{ number_format($detail->payable_amount, 2) }}</td>
                <td class="text-center">{{ $detail->number_of_days }}</td>
                <td class="text-center">
                  {{ \Carbon\Carbon::parse($detail->agreement_start_date)->format('d M Y') }}
                </td>
                <td class="text-center">
                  {{ \Carbon\Carbon::parse($detail->agreement_end_date)->format('d M Y') }}
                </td>
              </tr>
            @empty
              <tr>
          ...
          @if($details->isNotEmpty())
            <tfoot>
              <tr class="totals-row">
                ...
                <th class="text-end">{{ number_format($details->sum('salary_amount'), 2) }}</th>
      ...
      <div class="text-center mt-4 border-top pt-2">
        <small class="text-muted">© {{ now()->year }} Tadbeer Alebdaa. All rights reserved.</small>
      </div>
    </div>
  </div>
