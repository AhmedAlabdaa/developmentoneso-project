...
<body>
    ...
    <table class="dotted" style="margin:20px 0">
        <tr>
            <td>{{ $fmt($agreement->created_at) }}</td>
            <td>
        ...
        <div style="float:left;width:47%">
            <p style="margin:0;direction:ltr;text-align:left">
                <strong>Article&nbsp;1: Contract and Recruitment Office Fees</strong><br>
                    The second party agrees to pay the first party recruitment fees amounting to <strong>{{ number_format($totalAmount, 2) }}</strong> Dirhams. Payment will be made in
            </p>
        </div>
        <div style="float:left;width:47%;margin-left:2%">
            <p style="margin:0;direction:rtl;text-align:right">
                <strong>البند الأول: التعاقد واتعاب مكتب الاستقدام</strong><br>
                    تم الاتفاق على أن يدفع الطرف الثاني إلى الطرف الأول رسوم الاستقدام وقدرها <strong>{{ number_format($totalAmount, 2) }}</strong> درهم بالطريقة التالية:<br>
            </p>
        </div>
    ...
    <div style="margin:30px 0;overflow:auto">
        <div class="sig" style="float:left">
            ...
            <p><span style="font-weight:bold">Name&nbsp;:</span> <span style="border-bottom:1px solid #000;display:inline-block;width:75%">{{ $agreement->client->first_name }} {{ $agreement->client->last_name }}</span></p>
