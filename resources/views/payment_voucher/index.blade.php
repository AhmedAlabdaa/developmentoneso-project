          ...
          <table id="pv-table" class="table table-striped table-hover align-middle">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td class="text-end">{{ number_format($v->journal->total_debit ?? 0, 2) }}</td>
                <td class="text-end">{{ number_format($v->journal->total_credit ?? 0, 2) }}</td>
                <td>
                  @if(auth()->user()->role === 'Finance Officer')
                    <select class="form-select form-select-sm status-select"
                            data-id="{{ $v->id }}"
                            data-vno="{{ $v->voucher_no }}"
                            data-name="{{ $v->payee }}"
                            data-amount="{{ number_format($v->journal->total_debit ?? 0, 2, '.', '') }}"
                            data-current="{{ $v->status }}">
                  ...
                  @else
                    <span class="badge @if($v->status==='Approved') bg-success @elseif($v->status==='Cancelled') bg-danger @else bg-secondary @endif">{{ $v->status }}</span>
                  @endif
                </td>
                <td>{{ $firstName($v->created_by) }}</td>
                <td>{{ $firstName($v->approved_by) }}</td>
                <td>{{ $firstName($v->cancelled_by) }}</td>
                ...
                <td class="text-center">
                  <button class="btn btn-secondary btn-sm view-btn"
                          ...
                          data-id="{{ $v->id }}"
                          data-vno="{{ $v->voucher_no }}"
                          data-date="{{ $v->voucher_date->format('Y-m-d') }}"
                          data-payee="{{ $v->payee }}"
                          data-mode="{{ $v->mode_of_payment }}"
                          data-ref="{{ $v->reference_no }}"
                          data-narration="{{ $v->narration }}"
                          data-lines='@json($linesData)'
                          data-td="{{ number_format($v->journal->total_debit ?? 0, 2, '.', '') }}"
                          data-tc="{{ number_format($v->journal->total_credit ?? 0, 2, '.', '') }}">
                  ...
                  @endif
                  @if(auth()->user()->role === 'Finance Officer')
                  <button class="btn btn-danger btn-sm delete-btn" data-id="{{ $v->id }}"><i class="fa-solid fa-trash"></i></button>
                  @endif
                </td>
              </tr>
              @endforeach
...
<script>
  ...
  const storeUrl      = "{{ route('payment-vouchers.store') }}";
  const updateUrl     = "{{ route('payment-vouchers.update', ':id') }}";
  const destroyUrl    = "{{ route('payment-vouchers.destroy', ':id') }}";
  const statusUrl     = "{{ route('payment-vouchers.status', ':id') }}";
  const headerImg     = @json($headerImgUrl);
  const footerImg     = @json($footerImgUrl);
  const nextVoucherNo = @json($nextVoucherNo);
  const todayISO      = @json($todayISO); // "YYYY-MM-DD" from server in Asia/Dubai
