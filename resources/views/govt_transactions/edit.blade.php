      ...
      <div class="col-lg-12">
        <div class="card flex-fill">
          <div class="card-body" style="margin-top:10px">
            ...
            <form action="{{ route('govt-transactions.update', $invoice->invoice_number) }}" method="POST" enctype="multipart/form-data" class="row g-3">
              @csrf
              @method('PUT')
              ...
              <div class="col-md-4">
                ...
                <div class="input-group">
                  ...
                  <input type="text" name="mohre_ref" class="form-control" value="{{ old('mohre_ref', $invoice->mohre_ref) }}" required>
                </div>
              </div>
              ...
              <div class="col-md-4">
                ...
                <div class="input-group">
                  ...
                  <input type="text" class="form-control" value="{{ \Carbon\Carbon::parse(old('invoice_date', $invoice->invoice_date))->format('d M Y') }}" readonly>
                  <input type="hidden" name="invoice_date" value="{{ old('invoice_date', $invoice->invoice_date) }}">
                </div>
              </div>
              ...
              <div class="col-md-4">
                ...
                <div class="input-group">
                  ...
                  <select name="customer_type" id="customer_type" class="form-select" required>
                    <option value="Indoor" {{ old('customer_type', $invoice->customer_type)=='Indoor'?'selected':'' }}>Indoor Customer</option>
                    <option value="Walking" {{ old('customer_type', $invoice->customer_type)=='Walking'?'selected':'' }}>Walking Customer</option>
                  </select>
                </div>
              </div>
                ...
                <select name="customer_id" id="customer_id" class="form-select" required>
                  @foreach ($customers as $customer)
                    <option value="{{ $customer->id }}" {{ (string)old('customer_id', $invoice->customer_id)===(string)$customer->id?'selected':'' }}>
                      {{ $customer->first_name }} {{ $customer->last_name }} – {{ $customer->CL_Number ?? 'N/A' }}
                    </option>
                  @endforeach
                </select>
              </div>
              ...
              <div class="col-md-4" id="candidateCol">
                ...
                <select name="candidate_id" id="candidate_id" class="form-select">
                  ...
                  @foreach ($candidates as $candidate)
                    <option value="{{ $candidate->id }}" {{ (string)old('candidate_id', $invoice->candidate_id)===(string)$candidate->id?'selected':'' }}>
                      {{ $candidate->candidate_name }} – {{ $candidate->CN_Number ?? 'N/A' }}
                    </option>
                  @endforeach
                </select>
              </div>
              ...
              <div class="col-12">
                ...
                <table class="table table-bordered table-hover" id="invoiceItemsTable">
                  ...
                  <tbody id="invoiceItemsBody">
                    @if(old('service_name'))
                      @for($i=0;$i<count(old('service_name',[]));$i++)
                        <tr>
                          <td>
                            <select name="service_name[]" class="form-select service-select" required>
                              <option value="" disabled>Select Service</option>
                              @foreach($services as $service)
                                <option value="{{ $service->service_name }}" {{ old('service_name.'.$i)==$service->service_name?'selected':'' }}>{{ $service->service_name }}</option>
                              @endforeach
                              @if(old('service_name.'.$i) && collect($services)->pluck('service_name')->contains(old('service_name.'.$i))===false)
                                <option value="{{ old('service_name.'.$i) }}" selected>{{ old('service_name.'.$i) }}</option>
                              @endif
                            </select>
                          </td>
                          <td><input type="text" name="dw_numbers[]" class="form-control dw-number" value="{{ old('dw_numbers.'.$i) }}"></td>
                          <td><input type="number" name="quantities[]" class="form-control quantity" step="1" min="1" value="{{ old('quantities.'.$i,1) }}" required></td>
                          <td><input type="number" name="rates[]" class="form-control rate" step="0.01" min="0" value="{{ old('rates.'.$i,0) }}" required></td>
                          <td><input type="number" name="taxes[]" class="form-control tax" step="0.01" min="0" value="{{ old('taxes.'.$i,5) }}"></td>
                          <td><input type="number" name="center_fees[]" class="form-control center-fee" step="0.01" min="0" value="{{ old('center_fees.'.$i,0) }}"></td>
                          ...
                          <td class="text-center">
                            <button type="button" class="btn btn-danger btn-sm removeRowBtn" {{ $i===0?'disabled':'' }}><i class="fas fa-trash"></i></button>
                          </td>
                        </tr>
                      @endfor
                    @else
                        ...
                        <tr>
                          <td>
                            <select name="service_name[]" class="form-select service-select" required>
                              <option value="" disabled>Select Service</option>
                              @foreach($services as $service)
                                <option value="{{ $service->service_name }}" {{ $it->service_name==$service->service_name?'selected':'' }}>{{ $service->service_name }}</option>
                              @endforeach
                              @if($it->service_name && collect($services)->pluck('service_name')->contains($it->service_name)===false)
                                <option value="{{ $it->service_name }}" selected>{{ $it->service_name }}</option>
                              @endif
                            </select>
                          </td>
                          <td><input type="text" name="dw_numbers[]" class="form-control dw-number" value="{{ $it->dw_number }}"></td>
                          <td><input type="number" name="quantities[]" class="form-control quantity" step="1" min="1" value="{{ $it->qty ?? 1 }}" required></td>
                          <td><input type="number" name="rates[]" class="form-control rate" step="0.01" min="0" value="{{ $it->amount ?? 0 }}" required></td>
                          <td><input type="number" name="taxes[]" class="form-control tax" step="0.01" min="0" value="{{ $it->tax ?? 0 }}"></td>
                          <td><input type="number" name="center_fees[]" class="form-control center-fee" step="0.01" min="0" value="{{ $it->center_fee ?? 0 }}"></td>
                          <td><input type="text" class="form-control line-vat" value="0.00" readonly></td>
                          <td><input type="text" class="form-control line-total" value="0.00" readonly></td>
                          <td class="text-center">
                            <button type="button" class="btn btn-danger btn-sm removeRowBtn" {{ $idx===0?'disabled':'' }}><i class="fas fa-trash"></i></button>
                          </td>
                        </tr>
                      @endforeach
                    @endif
              ...
              <div class="col-md-2">
                ...
                <div class="input-group">
                  ...
                  <input type="number" step="0.01" min="0" id="discount_amount" name="discount_amount" class="form-control" value="{{ old('discount_amount', $invoice->discount_amount ?? 0) }}">
                </div>
              </div>
              ...
              <div class="col-md-2">
                ...
                <div class="input-group">
                  ...
                  <input type="number" step="0.01" min="0" id="received_amount" name="received_amount" class="form-control" value="{{ old('received_amount', $invoice->received_amount ?? 0) }}">
                </div>
              </div>
              ...
              <input type="hidden" name="currency" value="{{ old('currency', $invoice->currency ?? 'AED') }}">

              <div class="col-md-3">
                <label class="form-label">Payment Method <span class="text-danger">*</span></label>
                <select name="payment_mode" id="payment_mode" class="form-select" required>
                  @php $pm = old('payment_mode', $invoice->payment_mode); @endphp
                  <option value="Bank Transfer ADIB" {{ $pm=='Bank Transfer ADIB'?'selected':'' }}>Bank Transfer ADIB</option>
                  <option value="Bank Transfer ADCB" {{ $pm=='Bank Transfer ADCB'?'selected':'' }}>Bank Transfer ADCB</option>
                  <option value="POS-ID 60043758-ADIB" {{ $pm=='POS-ID 60043758-ADIB'?'selected':'' }}>POS-ID 60043758-ADIB</option>
                  <option value="POS-ID 60045161-ADCB" {{ $pm=='POS-ID 60045161-ADCB'?'selected':'' }}>POS-ID 60045161-ADCB</option>
                  <option value="ADIB-19114761" {{ $pm=='ADIB-19114761'?'selected':'' }}>ADIB-19114761</option>
                  <option value="ADIB-19136783" {{ $pm=='ADIB-19136783'?'selected':'' }}>ADIB-19136783</option>
                  <option value="Cash" {{ $pm=='Cash'?'selected':'' }}>Cash</option>
                  <option value="Cheque" {{ $pm=='Cheque'?'selected':'' }}>Cheque</option>
                  <option value="Replacement" {{ $pm=='Replacement'?'selected':'' }}>Replacement</option>
                </select>
              </div>

              <div class="col-md-3">
                <label class="form-label">Payment Reference</label>
                <input type="text" name="payment_reference" id="payment_reference" class="form-control" value="{{ old('payment_reference', $invoice->payment_reference) }}">
              </div>
              ...
              <div class="col-md-6">
                ...
                <div class="input-group">
                  ...
                  <input type="text" name="payment_note" id="payment_note" class="form-control" value="{{ old('payment_note', $invoice->payment_note) }}">
                </div>
              </div>

              <div class="col-md-12">
                <label class="form-label">Notes</label>
                <textarea name="notes" id="notes" class="form-control" rows="2">{{ old('notes', $invoice->notes) }}</textarea>
              </div>
