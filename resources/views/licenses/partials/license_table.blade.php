        ...
        <tbody>
            @foreach ($licenses as $license)
                <tr>
                    ...
                    <td>{{ $license->document_type }}</td>
                    <td>{{ $license->document_number }}</td>
                    <td>{{ $license->document_date ? \Carbon\Carbon::parse($license->document_date)->format('d F Y') : 'N/A' }}</td>
                    <td>{{ $license->expiry_date ? \Carbon\Carbon::parse($license->expiry_date)->format('d F Y') : 'N/A' }}</td>
                    <td>{{ $license->renewal_required ? 'Yes' : 'No' }}</td>
                    <td>{{ $license->status }}</td>
                    <td>{{ optional($license->uploadedByUser)->name ?? 'N/A' }}</td>
                    <td>{{ $license->uploaded_at ? \Carbon\Carbon::parse($license->uploaded_at)->format('d F Y h:i A') : 'N/A' }}</td>
                    <td class="actions">
                        <a href="javascript:void(0);" onclick="openFilePreview('{{ asset('storage/' . $license->file_path) }}')" class="btn btn-secondary btn-icon-only" title="View File">
                            <i class="fas fa-file"></i>
                        </a>
                        <a href="{{ asset('storage/' . $license->file_path) }}" class="btn btn-info btn-icon-only" title="Download">
                            <i class="fas fa-download"></i>
                        </a>
                        <a href="{{ route('licenses.show', $license->id) }}" class="btn btn-info btn-icon-only" title="View Details">
                            <i class="fas fa-eye"></i>
                        </a>
                        <a href="{{ route('licenses.edit', $license->id) }}" class="btn btn-warning btn-icon-only" title="Edit">
