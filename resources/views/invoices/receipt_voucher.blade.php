...
<head>
    ...
    <title>Receipt Voucher - {{ $invoice->invoice_number }}</title>
    ...
    <div class="voucher-container">
        ...
        <div class="header-section">
            ...
            <div class="voucher-meta">
                <p><strong>Receipt No:</strong> {{ $invoice->invoice_number }}</p>
...
@endphp
        ...
        <div style="margin-top: 30px;">
            <div class="content-row">
                ...
                <div class="value">{{ optional($invoice->customer)->first_name }} {{ optional($invoice->customer)->last_name }}</div>
            </div>

            <div class="content-row">
                <div class="label">The Sum of AED:</div>
                <div class="value">{{ number_format($voucherAmount, 2) }} ({{ ucwords(\NumberFormatter::create('en', \NumberFormatter::SPELLOUT)->format($voucherAmount)) }} Dirhams only)</div>
            </div>

            <div class="content-row">
                <div class="label">Being:</div>
                <div class="value">{{ $invoice->notes ?? 'Payment for services rendered' }}</div>
            </div>
...
@endphp
            ...
            <div class="content-row">
                ...
                <div class="value">{{ $paymentModeLabel }}</div>
            </div>

             @if($invoice->CN_Number)
            <div class="content-row">
                <div class="label">Ref (CN):</div>
                <div class="value">{{ $invoice->CN_Number }}</div>
            </div>
            @endif
        </div>
        ...
        <div class="d-flex justify-content-between align-items-end">
            <div class="amount-box">
                AED {{ number_format($voucherAmount, 2) }}
            </div>
        </div>
