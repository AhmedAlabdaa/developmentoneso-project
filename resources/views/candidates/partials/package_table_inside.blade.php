        ...
        <tbody>
            @forelse ($packages as $package)
                ...
                <tr
                    ...
                    data-cn="{{ strtoupper($package->CN_Number ?? '') }}"
                    data-candidate="{{ e($candidateTitle) }}"
                    data-nationality="{{ strtoupper($package->nationality ?? '') }}"
                    data-passport="{{ strtoupper($package->passport_no ?? '') }}"
                    data-passport-expiry="{{ $ppExpiry }}"
                    data-passport-expiry-human="{{ $ppExpiryHuman }}"
                    data-sponsor-name="{{ strtoupper($package->sponsor_name ?? '') }}"
                    data-sponsor-qid="{{ strtoupper($package->eid_no ?? '') }}"
                >
                    <td class="align-middle">
                        <a href="" class="text-decoration-none" style="color:#007bff">
                            {{ strtoupper($package->agreement_no) }}
                        </a>
                        @php $agStatus = $package->agreement?->status; @endphp
                    ...
                    <td class="align-middle text-center">
                        ...
                        <div class="text-muted small mt-1" title="{{ $package->sales_name }}">
                            {{ strtoupper(explode(' ', $package->sales_name)[0]) }}
                        </div>
                    </td>
                    ...
                    <td class="align-middle">
                        <a
                            ...
                            style="color:#007bff"
                        >
                            {{ $package->candidate_name ? strtoupper(\Illuminate\Support\Str::title(strtolower($package->candidate_name))) : 'N/A' }}
                        </a>

                        <img
                            src="{{ asset('assets/img/attach.png') }}"
                            alt="Attachment Icon"
                            style="cursor:pointer;margin-left:8px;vertical-align:middle;height:20px"
                            title="View Attachments of the Candidate"
                            onclick="showCandidateModal('{{ $package->candidate_name }}','{{ $package->id }}','{{ $package->reference_no }}')"
                        />

                        <div class="text-muted small mt-1">
                            {{ strtoupper($package->passport_no) }} — {{ strtoupper($package->nationality) }} — {{ strtoupper(\Illuminate\Support\Str::before($package->foreign_partner ?? '', ' ')) }} - {{ $package->CN_Number ?? ''}}
                        </div>

                        <div class="passport-pill ok">
                            <i class="fas fa-passport"></i>
                            <span class="pp-exp-state">{{ $ppExpiryHuman ? strtoupper($ppExpiryHuman) : 'N/A' }}</span>
                        </div>
                    </td>

                    <td class="align-middle">
                        <div class="status-stack">
                            <span class="btn btn-sm btn-{{ $outsideInsideStatus['class'] }} status-badge" style="font-size:10px;color:{{ $textColor($outsideInsideStatus['class']) }}">
                                <i class="fas fa-{{ $outsideInsideStatus['icon'] }}"></i> {{ strtoupper($outsideInsideStatus['label']) }}
                            </span>

                            @if($isOutside)
                                <span class="btn btn-sm btn-{{ $currentOutsideStatus['class'] }} status-badge" style="font-size:10px;color:{{ $textColor($currentOutsideStatus['class']) }}">
                                    <i class="fas fa-{{ $currentOutsideStatus['icon'] }}"></i> {{ strtoupper($currentOutsideStatus['label']) }}
                                </span>

                                <span class="btn btn-sm btn-{{ $insideStatus['class'] }} status-badge" style="font-size:10px;color:{{ $textColor($insideStatus['class']) }}">
                                    <i class="fas fa-{{ $insideStatus['icon'] }}"></i> {{ strtoupper($insideStatus['label']) }}
                                </span>
                            @else
                                <span class="btn btn-sm btn-{{ $insideStatus['class'] }} status-badge" style="font-size:10px;color:{{ $textColor($insideStatus['class']) }}">
                                    <i class="fas fa-{{ $insideStatus['icon'] }}"></i> {{ strtoupper($insideStatus['label']) }}
                    ...
                    <td class="align-middle text-center">
                        <div class="d-inline-flex flex-column align-items-center" style="gap:6px;min-width:170px">
                            <span class="btn btn-sm btn-{{ $btnClass }} px-2" style="font-size:10px;color:{{ $textColor($btnClass) }};white-space:nowrap">
                                <i class="fas fa-{{ $btnIcon }}"></i>
                                <span class="ms-1"> EXP: {{ strtoupper($expiryHuman) }}</span>
                            </span>

                            <div class="d-inline-flex align-items-center justify-content-center flex-wrap" style="gap:6px">
                                <span class="badge bg-{{ $metaBadgeClass }}" style="font-size:10px;color:{{ $metaTextColor }};padding:6px 8px;border-radius:999px;white-space:nowrap">
                                    <i class="fas fa-hourglass-end"></i>
                                    <span class="ms-1">{{ $isExpired ? 'OVER: '.$overDays.' D' : ($icaExpiry ? 'LEFT: '.$daysLeft.' D' : 'LEFT: N/A') }}</span>
                                </span>

                                <span class="badge bg-primary" style="font-size:10px;color:#fff;padding:6px 8px;border-radius:999px;white-space:nowrap">
                                    <i class="fas fa-coins"></i>
                                    <span class="ms-1">FINE: {{ number_format($liveFine, 0) }} AED</span>
                                </span>
                            </div>
                        </div>
                    </td>
                    ...
                    <td class="align-middle">
                        <a href="" class="text-decoration-none" style="color:#007bff">
                            {{ strtoupper($package->sponsor_name) }}
                        </a>
                        <div class="text-muted small mt-1">
                            CLIENT #: {{ strtoupper($package->CL_Number) }} — EID: {{ strtoupper($package->eid_no) }}
                        </div>
                    </td>

                    <td class="align-middle">
                        <div>{{ $package->wc_date ? \Carbon\Carbon::parse($package->wc_date)->format('d M Y') : 'N/A' }}</div>
                        <div class="text-muted small mt-1">
                            DW #: {{ strtoupper($package->dw_number ?? 'N/A') }}
                        </div>
                    </td>

                    <td class="align-middle">
                        <div>{{ $package->visa_date ? \Carbon\Carbon::parse($package->visa_date)->format('d M Y') : 'N/A' }}</div>
                        <div class="text-muted small mt-1">
                            {{ $package->visa_type ? strtoupper($package->visa_type) : 'N/A' }}
                        </div>
                    </td>

                    <td class="align-middle">
                        {{ $package->arrived_date ? \Carbon\Carbon::parse($package->arrived_date)->format('d M Y') : 'N/A' }}
                    </td>

                    <td class="align-middle">
                        <div>{{ $package->incident_date ? \Carbon\Carbon::parse($package->incident_date)->format('d M Y') : 'N/A' }}</div>
                        <div class="text-muted small mt-1">
                            {{ $package->incident_type ? strtoupper($package->incident_type) : 'N/A' }}
                        </div>
                    </td>

                    <td class="align-middle">
                        {{ \Carbon\Carbon::parse($package->updated_at)->format('d M Y h:i A') }}
                    </td>

                    <td class="align-middle">
                        {{ strtoupper($package->remarks) }}
                    </td>

                    <td class="align-middle" id="action-cell-{{ $package->id }}">
                        <a href="{{ route('package.edit', $package->id) }}" class="btn btn-primary btn-icon-only" title="Edit Package">
                            <i class="fas fa-edit"></i>
                        </a>

                        @if(in_array($package->inside_status, [0, 1]))
                            <a href="javascript:void(0)" class="btn btn-success btn-icon-only" onclick="openConsentModal('{{ $package->id }}','{{ $package->candidate_name }}')" title="Consent">
                                <i class="fas fa-exchange-alt"></i>
                            </a>
                        @endif

                        @if($package->inside_status === 0 || $package->inside_status === 6)
                            <a href="javascript:void(0);" class="btn btn-primary btn-icon-only" onclick="openDropdown('{{ $package->id }}', this, '{{ $package->candidate_name }}')" title="Change Status">
                                <i class="fas fa-train"></i>
                            </a>

                            <div class="dropdown-container" id="dropdownContainer-{{ $package->id }}">
                                ...
                                <div class="dropdown-header">
                                    ...
                                    <p>Change status for <span class="candidate-name">{{ $package->candidate_name }}</span></p>
                                </div>

                                <select
                                    class="form-control status-dropdown"
                                    data-original="{{ $package->inside_status }}"
                                    onchange="confirmStatusChange(this,'{{ $package->id }}','{{ $package->candidate_name }}')"
                                >
                                    @php
                                        $allowed = [
                                            0 => 'Change Status',
                                    ...
                                    @foreach($allowed as $id => $name)
                                        <option value="{{ $id }}" @selected($package->inside_status == $id)>{{ $name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        @endif

                        @if(in_array($package->inside_status, [1]))
                            <a href="{{ route('package.exit', $package->cn_number_series) }}" class="btn btn-danger btn-icon-only" title="Exit">
                                <i class="fas fa-sign-out-alt"></i>
                            </a>
                        @endif

                        <a href="{{ route('package.showCV', ['package' => $package->cn_number_series]) }}" target="_blank" class="btn btn-info btn-icon-only ms-1" title="View CV">
...
(() => {
    ...
    const routes={
        transferToEmployees:'{{ route('transfer-to-employees') }}',
        officeData:"{{ route('package.officeData' , ':id') }}",
        officeSave:"{{ route('package.officeSave') }}",
        incidentSave:"{{ route('package.incidentSave') }}",
        insideStatusUpdate:"{{ route('packages.update-status-inside', ['packageId' => ':id']) }}"
    };
