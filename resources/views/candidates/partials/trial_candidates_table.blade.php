                        ...
                        <td class="actions">
                            ...
                            <div class="dropdown-container" id="dropdownContainer-{{ $trial->id }}" style="display: none;">
                                ...
                                <select class="form-control status-dropdown"
                                        id="statusDropdown-{{ $trial->id }}"
                                        name="current_status"
                                        onchange="trialChangeStatus(this, {{ $trial->id }}, {{ $trial->candidate_id }}, {{ $trial->client_id }})">
                                    @if ($trial->trial_status === "Active")
                            ...
                            <a href="{{ route($trial->trial_type . '.exit', ['reference_no' => $trial->reference_no]) }}" class="btn btn-primary btn-icon-only" title="Exit Form" target="_blank">
                                <i class="fas fa-file-export"></i>
                            </a>
                            @if (Auth::user()->role === 'Admin')
                                <form action="{{ route('candidates.destroy', $trial->reference_no) }}" method="POST" style="display:inline;" id="delete-form-{{ $trial->reference_no }}">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" class="btn btn-danger btn-icon-only" onclick="confirmDelete('{{ $trial->reference_no }}')" title="Delete">
...
<nav aria-label="Pagination">
    <div class="pagination-container">
        <span class="muted-text">Showing {{ $trials->firstItem() }} to {{ $trials->lastItem() }} of {{ $trials->total() }} results</span>
...
<script type="text/javascript">
    ...
    function updateTrialStatus(status, trialId, candidateId, clientId) {
        ...
        const updateStatusUrl = "{{ route('employees.trial-status', ['candidate' => ':reference_no']) }}".replace(':reference_no', candidateId);
        showPreloader();
        let ajaxUrl = '';
        let modalId = '';
        ...
        switch (status) {
            case "Confirmed":
                ...
                break;
            case "Change Status":
                ajaxUrl = '{{ route('candidates.getChangeStatusDetails') }}';
    ...
    $('#saveConfirmedModalButton').on('click', function () {
        ...
        $.ajax({
            url: '{{ route('candidates.saveTrialConfirmed') }}',
            type: 'POST',
            headers: { 'X-CSRF-TOKEN': csrfToken },
            data: formData,
            processData: false,
    ...
    document.getElementById('saveChangeStatusModalButton').addEventListener('click', function () {
        ...
        fetch('{{ route("candidates.updateChangeStatus") }}', {
            method: 'POST',
            headers: {
    ...
    document.getElementById('saveTrialReturnButton').addEventListener('click', function () {
        ...
        const formData = new FormData(form);
        button.disabled = true;
        fetch('{{ route("candidates.saveTrialReturn") }}', {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
            },
    ...
    document.getElementById('saveSalesReturnButton').addEventListener('click', function () {
        ...
        const formData = new FormData(form);
        button.disabled = true;
        fetch('{{ route("candidates.saveSalesReturn") }}', {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
            },
    ...
    document.getElementById('saveIncidentButton').addEventListener('click', function () {
        ...
        const formData = new FormData(form);
        button.disabled = true;
        fetch('{{ route("candidates.saveReturnIncident") }}', {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
            },
