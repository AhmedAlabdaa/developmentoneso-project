        ...
        <tbody>
            ...
            @foreach ($candidates as $candidate)
                <tr class="{{ $candidate->appeal == 1 ? 'table-warning appeal-row' : '' }}">
                    ...
                    <td><button class="btn btn-primary">{{ strtoupper(\Carbon\Carbon::parse($candidate->departure_date)->format('d M Y')) }}</button></td>
                    ...
                    <td>{{ strtoupper($candidate->pcc_date) }}</td>
                    <td>{{ strtoupper($candidate->pcc_status) }}</td>
                    <td>{{ strtoupper($candidate->embassy_submitted_date) }}</td>
                    <td>{{ strtoupper($candidate->embassy_released_date) }}</td>
                    <td>{{ strtoupper($candidate->insurance_date) }}</td>
                    <td>{{ strtoupper($candidate->insurance_status) }}</td>
                    @php
                    ...
                    @endphp
                    <td class="actions">
                        <a href="/{{ $candidate->reference_no }}/cv" target="_blank" class="btn btn-info btn-icon-only ms-1" title="View CV">
                        ...
                        <div class="dropdown-container" id="dropdownContainer-{{ $candidate->id }}" style="display: none;">
                            ...
                            <select class="form-control status-dropdown"
                                    id="statusDropdown-{{ $candidate->id }}"
                                    name="current_status"
                                    onchange="confirmStatusChange(this, '{{ $candidate->id }}', '{{ $candidate->candidate_name }}')">
