        ...
        <tbody>
            ...
            @else
                @foreach ($candidates as $candidate)
                    <tr class="{{ $candidate->appeal == 1 ? 'table-warning appeal-row' : '' }}" data-ref="{{ $candidate->reference_no }}">
                        <td>{{ $candidate->reference_no }}</td>
                        <td>
                        ...
                        <td class="js-status-cell" data-status="{{ (int) $candidate->current_status }}">
                            <i class="fas {{ $icon }} text-{{ $color }}"></i>
                            <span class="js-status-label">{{ $label }}</span>
                        </td>
                        ...
                        <td class="align-middle">
                            ...
                            <img
                                ...
                                onclick="showCandidateModal('{{ $candidate->candidate_name }}', '{{ $candidate->id }}', '{{ $candidate->reference_no }}')"
                            />
                        </td>
                        ...
                        <td class="actions">
                            ...
                            @if (in_array((int) $candidate->current_status, [0, 1], true))
                                <button type="button" class="btn btn-{{ (int) $candidate->current_status === 0 ? 'success' : 'warning' }} btn-icon-only ms-1 js-draft-toggle" data-url="{{ route('candidates.toggleDraft', ['candidateId' => $candidate->id]) }}" data-current="{{ (int) $candidate->current_status }}" title="{{ (int) $candidate->current_status === 0 ? 'Un-Draft (Make Available)' : 'Draft (Make Hidden)' }}">
                                    <i class="fas {{ (int) $candidate->current_status === 0 ? 'fa-folder-open' : 'fa-file' }}"></i>
                                </button>
                            @endif
                            ...
                            @endif
                            @if (in_array(Auth::user()->role, ['Sales Officer', 'Sales Coordinator']) && (int) $candidate->current_status == 5)
...
<script>
    (function (w, $) {
        ...
        App.config.csrf = App.config.csrf || @json(csrf_token());
        App.config.cvRouteTemplate = App.config.cvRouteTemplate || @json(route('candidates.showCV', ['candidate' => '___REF___']));
