{{-- resources/views/contracts/show2.blade.php --}}
@php
use Carbon\Carbon;
...
<head>
  <meta charset="UTF-8">
  <title>Contract of {{ $profile->first_name }} {{ $profile->last_name }}</title>
  <style>
...
<body>
  <div class="contract-container">
    ...
    <table class="table-container">
      <tr>
        <td><input type="text" value="{{ $createdAt }}"></td>
        <td><strong>({{ $packageLabelAr }}) اتفاقية تشغيل عامل مساعد<br>Domestic Worker Contract ({{ $packageLabelEn }})</strong></td>
        <td><input type="text" readonly value="{{ $contract->reference_no }}"></td>
        <td><strong>Contract Number<br>اتفاقية رقم</strong></td>
      </tr>
    </table>
    <table class="solid-table-container">
      <tr>
        <td><input type="text" value="{{ $agreement->client->first_name }} {{ $agreement->client->last_name }}"></td>
      ...
      <tr>
        ...
        <td>
          {{ $contract->replaced_by_name ?: $agreement->candidate_name }}
        </td>
      ...
      <tr>
        <td><input type="text" value="{{ $agreement->client->email }}"></td>
        <td><strong>البريد الإلكتروني<br>EMAIL</strong></td>
        <td><input type="text" value="{{ $agreement->Nationality->name ?? $agreement->nationality }}"></td>
        <td><strong>الجنسية<br>Nationality</strong></td>
      </tr>
      <tr>
        <td><input type="text" value="{{ Carbon::parse($agreement->agreement_start_date)->format('d-m-Y') }}"></td>
        <td><strong>تاريخ بدء الاتفاقية<br>Agreement Start Date</strong></td>
        <td><input type="text" value="{{ Carbon::parse($agreement->agreement_end_date)->format('d-m-Y') }}"></td>
    ...
    <div class="terms-container">
      <div class="terms">
        <ul>
          ...
          <li>The First Party commits to providing a domestic helper service under the sponsorship of the First Party, who will handle the work permit and residency for the domestic helper for a certian period in exchange for an upfront fee agreed upon as: <strong>{{ $totalAmt }} AED</strong>.</li>
          <li>The parties agree that the duration of this contract shall extend from the date <strong>{{ $agreementStart }}</strong> to <strong>{{ $agreementEnd }}</strong>.</li>
        ...
        <ul class="rtl">
          ...
          <li>يلتزم الطرف الأول بتوفير خدمة عاملةٍ منزليةٍ تحت كفالته، ويتعهد باستخراج تصريح العمل والإقامة للعاملة لمدة محددة، وذلك مقابل رسم مقدَّم متفق عليه وقدره: <strong>{{ $totalAmt }} درهم إماراتي</strong>.</li>
          <li>يتفق الطرفان على أن مدة هذا العقد تمتد من تاريخ <strong>{{ $agreementStart }}</strong> إلى تاريخ <strong>{{ $agreementEnd }}</strong>.</li>
  ...
  @foreach($replacementHistories as $i => $hist)
    <div class="page-break"></div>
    <div class="header">
      <img src="{{ asset('assets/img/' . $headerFileName) }}" alt="Header Image">
    </div>
    <p class="ref">Ref: {{ $contract->reference_no }}-AN{{ $i+1 }}</p>
    <h2 class="title-ar">ملحق رقم ({{ $i+1 }}) – بيانات العامل وتاريخ بدء العمل</h2>
    <h2 class="title-en">Annex No. {{ $i+1 }} – Worker and Contract details</h2>
    <table class="history_table">
      <tr>
        <th style="width:20%;">Worker Name</th>
        <td style="width:60%;">{{ $hist->name }}</td>
      ...
      <tr>
        ...
        <td>{{ Carbon::parse($hist->replacement_date)->format('d M Y') }}</td>
        <th>تاريخ بدء العمل</th>
      </tr>
      <tr>
        <th>Contract Expiry date</th>
        <td>{{ Carbon::parse($contract->contract_end_date)->format('d M Y') }}</td>
        <th>تاريخ نهاية العقد</th>
      </tr>
    </table>
    <div class="row">
      <div class="col-lg-6" dir="rtl">
        يعتبر هذا الملحق جزءاً لا يتجزأ من اتفاقية رقم  {{ $contract->reference_no }}، ويقر الطرفان ويوافقان على التفاصيل الواردة فيه.
      </div>
      <div class="col-lg-6">
        This annex is an integral part of Agreement No. {{ $contract->reference_no }}, and both parties acknowledge and agree to the details provided herein.
      </div>
    </div>
    ...
    <div class="sig-grid">
      ...
      <div class="sig-block" dir="rtl">
        ...
        <div><label>الاسم / Name: {{ $agreement->client->first_name }} {{ $agreement->client->last_name }}</label><span class="field"></span></div>
        <div><label>التوقيع / Signature:</label><span class="field"></span></div>
      </div>
    </div>
    <div class="footer" style="margin-top:20px;">
