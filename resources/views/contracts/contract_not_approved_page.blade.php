    ...
    <div class="card">
      <div class="card-body">
        ...
        <div class="action-buttons">
          <a href="{{ route('contracts.index') }}" class="btn btn-primary btn-sm">
            <i class="fas fa-arrow-left"></i> Back to Contracts
          </a>
          @php $isApproved = ($contract->marked === 'Yes'); @endphp
        ...
        <div class="contract-info text-start">
          ...
          <strong>{{ $contract->reference_no }}</strong>
        </div>
        ...
        <div class="table-responsive mt-3">
          <table class="table table-striped table-hover">
            ...
            <tbody>
              <tr>
                <td>{{ strtoupper($contract->candidate_name) }}</td>
                <td>{{ strtoupper(optional($contract->client)->first_name . ' ' . optional($contract->client)->last_name) }}</td>
...
<script>
  (function () {
    ...
    const refNo = @json($contract->reference_no);
    ...
    btn.addEventListener('click',function(){
      ...
      setLoading(true);
      fetch(`{{ route('contracts.update-marked') }}`,{
        method:'POST',
