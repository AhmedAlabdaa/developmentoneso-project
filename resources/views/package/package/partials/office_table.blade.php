  ...
  <table class="table table-striped table-hover">
    ...
    <tbody>
      @forelse ($packages as $p)
        ...
        <tr id="row-{{ $p->id }}" data-held="{{ $isHeld ? '1' : '0' }}">
          <td>
            <a href="#" class="text-primary" target="_blank">{{ strtoupper($p->CN_Number) }}</a>
          </td>
          ...
          <td>
            <button
              ...
              style="font-size:8px"
              data-can-change="{{ $canOverrideHold ? '1' : '0' }}"
              data-candidate-id="{{ $CNDID }}"
              data-current-id="{{ $currentSalesId }}"
              data-current-label="{{ $currentSalesDisplay }}"
            >
              <i class="fas fa-user-edit"></i>
              <span class="js-sales-label">{{ $currentSalesDisplay }}</span>
            </button>
          </td>

          <td title="{{ $candidateFull }}" class="candidate-col">
            <a href="#" target="_blank" class="text-decoration-none">{{ $candidateDisplay }}</a>

            @if($isHeld)
              <div class="js-hold-meta">
                <span class="hold-meta">
                  <i class="fas fa-hand-paper"></i>
                  <span class="js-hold-dt">{{ $holdDt }}</span>
                  <small class="js-hold-sales">{{ $holdSalesFirst }}</small>
                </span>
              </div>
            @else
          ...
          <td title="{{ strtoupper($p->foreign_partner ?? '') }}">
            {{ $agentShort !== '' ? $agentShort : 'N/A' }}
          </td>

          <td title="{{ $clientFull }}">
            <a href="{{ $clientUrl }}" class="text-primary" target="_blank">{{ $clientDisplay }}</a>
          </td>
          ...
          <td>
            @if(!$isHeld)
              <a
                ...
                title="Change Status"
                onclick="openDropdown('{{ $p->id }}', this, '{{ $candidateNice }}')"
              >
                <i class="fas fa-train"></i>
              </a>

              <div class="dropdown-container" id="dropdownContainer-{{ $p->id }}" style="display:none">
                ...
                <div class="dropdown-header">
                  ...
                  <p>package <span id="packageName-{{ $p->id }}" class="package-name"></span>?</p>
                </div>
                ...
                <select
                  ...
                  data-cn="{{ $p->CN_Number }}"
                  data-candidate="{{ $candidateNice }}"
                  data-held="0"
                  data-can-override-hold="{{ $canOverrideHold ? '1' : '0' }}"
                  onchange="confirmStatusChange(this,'{{ $p->id }}','{{ $candidateNice }}')"
                >
                  @foreach($allowedStatuses as $statusId => $statusName)
                    <option value="{{ $statusId }}" {{ (int)$p->_status === (int)$statusId ? 'selected' : '' }}>
                      {{ $statusName }}
                    </option>
                  @endforeach
                </select>
            ...
            @if($transferAllowedRow)
              <a
                ...
                data-candidate-name="{{ $candidateNice }}"
                data-target-company="{{ $transferTargetName }}"
              >
            ...
            <a
              href="{{ route('package.exit', ['reference_no' => $p->hr_ref_no]) }}"
              class="btn btn-primary btn-icon-only"
              title="Exit Form"
              target="_blank"
            >
                ...
                <div class="col-md-4">
                  ...
                  <select id="officePaymentMethod" name="office_payment_method" class="form-control" required>
                    ...
                    @foreach($methods as $code)
                      @php $code = strtoupper($code); $label = $all[$code] ?? $code; @endphp
...
(() => {
  ...
  const APP_TENANT = '{{ $appTenant ?? '' }}';

  const routes = {
    updateStatus: "{{ route('packages.update-status-inside',['packageId'=>'__PACKAGE_ID__']) }}",
    updateMonthlyStatus: "{{ route('packages.update-status-monthly') }}",
    agreementsInside: "{{ route('agreements.insideagreement') }}",
    incidentDataOffice: "{{ route('package.incidentDataOffice',':id') }}",
    incidentSave: "{{ route('package.officeIncidentSave') }}",
    updateSalesName: "{{ route('packages.updateSalesName') }}",
    transferCandidate: "{{ route('packages.transfer') }}",
    updateHold: "{{ route('office.updateHold') }}"
  };
