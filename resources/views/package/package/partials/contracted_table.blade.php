    ...
    <tbody>
      @forelse ($packages as $p)
        <tr>
          ...
          <td>{{ $p->foreign_partner ? strtoupper(preg_split('/\s+/', trim($p->foreign_partner))[0]) : '-' }}</td>
          <td>{{ strtoupper($p->sponsor_name ?? '-') }}</td>
          <td>{{ strtoupper($p->eid_no ?? '-') }}</td>
          <td>
            {{ !empty($p->trial_start_date) ? \Carbon\Carbon::parse($p->trial_start_date)->format('d M Y') : '-' }}
          </td>
          <td>
            {{ !empty($p->trial_end_date) ? \Carbon\Carbon::parse($p->trial_end_date)->format('d M Y') : '-' }}
          </td>
          <td>{{ strtoupper($p->sales_name ?? '-') }}</td>
          <td>
            @if($p->updated_at)
              {{ \Carbon\Carbon::parse($p->updated_at)->format('d M Y h:i A') }}
            @else
              -
            @endif
          </td>
          <td>
            <button type="button" class="btn btn-primary btn-icon-only" title="Actions" onclick="openDropdown('{{ $p->trial_id }}', this, '{{ $p->candidate_name }}')">
              <i class="fas fa-ellipsis-v"></i>
            </button>
            <div class="dropdown-container" id="dropdownContainer-{{ $p->trial_id }}">
              ...
              <select class="form-control status-dropdown" data-candidate-id="{{ $p->trial_id }}" data-cn="{{ $p->CN_Number }}" data-passport="{{ $p->passport_no }}" data-candidate-name="{{ $p->candidate_name }}" onchange="handleActionChange(this)">
        ...
        <div class="modal-body">
          <div class="row g-3 mb-2">
            ...
            <div class="col-md-4">
              ...
              <input type="date" class="form-control js-sales-return-date" name="return_date" value="{{ \Carbon\Carbon::now('Asia/Qatar')->format('Y-m-d') }}">
            </div>
          </div>
...
var incidentDataUrl   = "{{ route('package.incidentData', ':id') }}";
var incidentSaveUrl   = "{{ route('package.incidentSave') }}";
var refundViewUrl     = "{{ route('packages.refund-sales-view') }}";
var replacementViewUrl= "{{ route('packages.replacement-sales-view') }}";
var saveSalesReturnUrl= "{{ route('packages.saveSalesReturn') }}";
