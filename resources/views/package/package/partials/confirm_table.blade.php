      ...
      @else
        @foreach($packages as $trial)
          ...
          <tr style="{{ $rowStyle }}">
            ...
            <td>
              <button type="button" style="font-size:8px!important" class="btn btn-outline-primary btn-sm js-open-sales-modal" data-can-change="{{ $canChange ? '1':'0' }}" data-candidate-id="{{ $cid }}" data-trial-id="{{ $tid }}" data-current-id="{{ $currentSalesId }}" data-current-label="{{ $currentSalesDisplay }}">
            ...
            <td>
              <div class="icon-bar">
                ...
                @if ($trial->trial_status === 'Trial Return')
                  <a class="icon-btn" href="{{ route('packages.trialReturnForm',['agreement_reference_no'=>$trial->agreement_reference_no]) }}" target="_blank" data-bs-toggle="tooltip" title="Trial Return Form"><i class="fas fa-file-invoice"></i></a>
                @endif
                <button class="icon-btn js-open-status-dropdown" data-bs-toggle="tooltip" title="Change Status" data-trial-id="{{ $tid }}"><i class="fas fa-train"></i></button>
              </div>
              <div class="dropdown-container" id="dropdownContainer-{{ $tid }}">
