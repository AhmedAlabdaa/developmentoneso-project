...
<div id="salary-slip-print">
  <div class="salary-slip-wrapper">
    <div class="salary-slip-meta">
      ...
      <span>Reference No.: {{ $headerRef }}</span>
    </div>
    ...
    <table class="salary-slip-table">
      ...
      @if(isset($rows) && $rows && method_exists($rows, 'count') && $rows->count())
        @foreach($rows as $row)
          @php
          ...
          <tr>
            ...
            <td class="salary-slip-text-center">{{ $method }}</td>
    ...
    <div class="salary-slip-ack">
      I, <strong>{{ $candidateName }}</strong>, hereby acknowledge that I have received the above-mentioned salary amount from Company.
    </div>
