      ...
      <div class="card">
        <div class="card-hdr">
          <div>
            ...
            <div class="sub">
              CN: {{ $refund->cn_number ?? '-' }}
              • Candidate: {{ $refund->candidate_name ?? '-' }}
              • Type: {{ ucfirst($refund->action_type ?? '-') }}
            </div>
          </div>
          <div class="btns">
            <a href="{{ route('refunds.show',['refund'=>$refund->id]) }}" class="btn btn-outline-secondary">
        ...
        <form action="{{ route('refunds.update',['refund'=>$refund->id]) }}" method="POST" enctype="multipart/form-data">
          @csrf
          @method('PUT')
          ...
          <div class="grid">
            <div>
              ...
              <select name="action_type" class="form-select js-action-type">
                <option value="refund" {{ $val('action_type')==='refund'?'selected':'' }}>Refund</option>
                <option value="replacement" {{ $val('action_type')==='replacement'?'selected':'' }}>Replacement</option>
              </select>
            </div>
            <div>
              ...
              <select name="status" class="form-select">
                ...
                <option value="approved" {{ $val('status')==='approved'?'selected':'' }}>Approved</option>
                <option value="rejected" {{ $val('status')==='rejected'?'selected':'' }}>Rejected</option>
              </select>
            </div>
            <div>
              <label class="form-label">Refund Type</label>
              <input type="text" name="refund_type" class="form-control" value="{{ $val('refund_type') }}">
            </div>
            <div>
              <label class="form-label">Stage</label>
              <input type="text" name="refund_stage" class="form-control" value="{{ $val('refund_stage') }}">
            </div>
          </div>
          ...
          <div class="grid">
            <div>
              ...
              <input type="text" name="cn_number" class="form-control" value="{{ $val('cn_number') }}">
            </div>
            <div>
              <label class="form-label">Agreement Reference No.</label>
              <input type="text" name="agreement_reference_no" class="form-control" value="{{ $val('agreement_reference_no') }}">
            </div>
            <div>
              <label class="form-label">Candidate Name</label>
              <input type="text" name="candidate_name" class="form-control" value="{{ $val('candidate_name') }}" readonly>
            </div>
            <div>
              <label class="form-label">Sponsor Name</label>
              <input type="text" name="client_name" class="form-control" value="{{ $val('client_name') }}" readonly>
            </div>
            <div>
            ...
            <div>
              ...
              <input type="date" name="contract_start_date" class="form-control js-start" value="{{ $valDate('contract_start_date') }}">
            </div>
            <div>
              <label class="form-label">Return Date</label>
              <input type="date" name="return_date" class="form-control js-return" value="{{ $valDate('return_date') }}">
            </div>
            <div>
              <label class="form-label">Number Of Days</label>
              <input type="number" name="number_of_days" class="form-control js-days" value="{{ $valNum('number_of_days') ?: 0 }}">
            </div>
            <div>
              <label class="form-label">Worked Days With Sponsor</label>
              <input type="number" name="maid_worked_days" class="form-control js-worked-days" value="{{ $valNum('maid_worked_days') ?: 0 }}">
            </div>
            <div>
              <label class="form-label">Worker Monthly Salary</label>
              <input type="number" step="0.01" name="worker_salary_amount" class="form-control js-salary-amount" value="{{ $valNum('worker_salary_amount') ?: '0.00' }}">
            </div>
            <div>
              <label class="form-label">Worker Salary Status</label>
              <select name="worker_salary_type" class="form-select js-salary-type">
                <option value="">Select</option>
                @foreach($salaryTypes as $opt)
                  <option value="{{ $opt }}" {{ $val('worker_salary_type')===$opt?'selected':'' }}>{{ $opt }}</option>
                @endforeach
              </select>
            </div>
            <div>
          ...
          <div class="grid">
            <div>
              ...
              <select name="original_passport" class="form-select">
                <option value="">Select</option>
                @foreach($passportWhere as $opt)
                  <option value="{{ $opt }}" {{ $val('original_passport')===$opt?'selected':'' }}>{{ $opt }}</option>
                @endforeach
              </select>
            </div>
            <div>
              ...
              <select name="worker_belongings" class="form-select">
                <option value="">Select</option>
                @foreach($belongingsWhere as $opt)
                  <option value="{{ $opt }}" {{ $val('worker_belongings')===$opt?'selected':'' }}>{{ $opt }}</option>
                @endforeach
              </select>
            </div>
            ...
            <div class="js-refund-only">
              ...
              <select class="form-select js-refund-reason">
                ...
                @foreach($refundReasons as $r)
                  <option value="{{ $r }}" {{ strtoupper($val('refund_reason'))===strtoupper($r)?'selected':'' }}>{{ $r }}</option>
                @endforeach
                <option value="Other">Other</option>
              </select>
              <input type="text" class="form-control mt-2 d-none js-refund-reason-other" placeholder="Type other reason">
              <input type="hidden" name="refund_reason" class="js-refund-reason-final" value="{{ $val('refund_reason') }}">
            </div>
            ...
            <div class="js-replacement-only">
              ...
              <select class="form-select js-replacement-reason">
                <option value="">Select</option>
                @foreach($refundReasons as $r)
                  <option value="{{ $r }}" {{ strtoupper($val('replacement_reason'))===strtoupper($r)?'selected':'' }}>{{ $r }}</option>
                @endforeach
                <option value="Other">Other</option>
              </select>
              <input type="text" class="form-control mt-2 d-none js-replacement-reason-other" placeholder="Type other reason">
              <input type="hidden" name="replacement_reason" class="js-replacement-reason-final" value="{{ $val('replacement_reason') }}">
            </div>

            <div class="js-refund-only">
              <label class="form-label">Refund Due Date</label>
              <input type="date" name="refund_due_date" class="form-control" value="{{ $valDate('refund_due_date') }}">
            </div>

            <div class="js-replacement-only">
              <label class="form-label">Replacement Due Date</label>
              <input type="date" name="replacement_due_date" class="form-control" value="{{ $valDate('replacement_due_date') }}">
            </div>
          </div>
          ...
          <div class="grid">
            <div>
              ...
              <input type="number" step="0.01" name="contracted_amount" class="form-control" value="{{ $valNum('contracted_amount') }}">
            </div>

            <div class="js-refund-only">
              <label class="form-label">Refund Balance</label>
              <input type="number" step="0.01" name="refund_balance" class="form-control" value="{{ $valNum('refund_balance') }}">
            </div>

            <div class="js-replacement-only">
              <label class="form-label">Replacement Balance</label>
              <input type="number" step="0.01" name="replacement_balance" class="form-control js-replacement-balance" value="{{ $valNum('replacement_balance') }}">
            </div>

            <div class="js-refund-only">
              <label class="form-label">Refund Final Balance</label>
              <input type="number" step="0.01" name="refund_final_balance" class="form-control" value="{{ $valNum('refund_final_balance') }}">
            </div>

            <div class="js-replacement-only">
              <label class="form-label">Replacement Final Balance</label>
              <input type="number" step="0.01" name="replacement_final_balance" class="form-control js-final" value="{{ $valNum('replacement_final_balance') }}">
            </div>

            <div class="js-replacement-only js-bank-amount-wrap {{ $val('worker_salary_type')==='Customer paid to Office by Bank Transfer'?'':'d-none' }}">
              <label class="form-label">Bank Transfer Amount</label>
              <input type="number" step="0.01" name="replacement_bank_amount" class="form-control js-bank-amount" value="{{ $valNum('replacement_bank_amount') ?: '0.00' }}">
            </div>

            <div>
              <label class="form-label">Salary Deduction Amount</label>
              <input type="number" step="0.01" name="salary_deduction_amount" class="form-control" value="{{ $valNum('salary_deduction_amount') }}">
            </div>
          </div>
          ...
          <div class="grid">
            <div>
              ...
              <select name="payment_method" class="form-select">
                <option value="">Select</option>
                <option value="Cash" {{ $val('payment_method')==='Cash'?'selected':'' }}>Cash</option>
                <option value="Bank Transfer" {{ $val('payment_method')==='Bank Transfer'?'selected':'' }}>Bank Transfer</option>
                <option value="Cheque" {{ $val('payment_method')==='Cheque'?'selected':'' }}>Cheque</option>
              </select>
            </div>
            <div>
              <label class="form-label">Paid Amount</label>
              <input type="number" step="0.01" name="paid_amount" class="form-control" value="{{ $valNum('paid_amount') }}">
            </div>
            <div>
              <label class="form-label">Paid At</label>
              <input type="datetime-local" name="paid_at" class="form-control" value="{{ $valDT('paid_at') }}">
            </div>
            <div>
              <label class="form-label">Payment Reference</label>
              <input type="text" name="payment_reference" class="form-control" value="{{ $val('payment_reference') }}">
            </div>
          </div>
          ...
          <div class="grid">
            <div>
              ...
              <input type="text" name="approved_by" class="form-control" value="{{ $val('approved_by') }}">
            </div>
            <div>
              <label class="form-label">Approved At</label>
              <input type="datetime-local" name="approved_at" class="form-control" value="{{ $valDT('approved_at') }}">
            </div>
            <div class="grid-1" style="padding:0">
              <label class="form-label">Approved Note / Remarks</label>
              <textarea name="approved_note" class="form-control" rows="3">{{ $val('approved_note') }}</textarea>
            </div>
            <div>
              <label class="form-label">NOC Status</label>
              <input type="text" name="noc_status" class="form-control" value="{{ $val('noc_status') }}">
            </div>
            <div>
              <label class="form-label">NOC Expiry Date</label>
              <input type="date" name="noc_expiry_date" class="form-control" value="{{ $valDate('noc_expiry_date') }}">
            </div>
          </div>
          ...
          <div class="grid row-soft">
            <div>
              ...
              @if($fileUrl($refund->bank_proof_path ?? null))
                <div class="preview mt-2">
                  <a target="_blank" href="{{ $fileUrl($refund->bank_proof_path) }}" class="btn btn-sm btn-outline-primary">Open</a>
                  @if($isImage($refund->bank_proof_path))
                    <img src="{{ $fileUrl($refund->bank_proof_path) }}" class="thumb" alt="">
                  @endif
                </div>
              @endif
            ...
            <div>
              ...
              @if($fileUrl($refund->proof_path ?? null))
                <div class="preview mt-2">
                  <a target="_blank" href="{{ $fileUrl($refund->proof_path) }}" class="btn btn-sm btn-outline-primary">Open</a>
                  @if($isImage($refund->proof_path))
                    <img src="{{ $fileUrl($refund->proof_path) }}" class="thumb" alt="">
                  @endif
                </div>
              @endif
              <div class="hint mt-1">Keeps existing if empty</div>
            ...
            <div>
              ...
              @if($fileUrl($refund->penalty_payment_proof_path ?? null))
                <div class="preview mt-2">
                  <a target="_blank" href="{{ $fileUrl($refund->penalty_payment_proof_path) }}" class="btn btn-sm btn-outline-primary">Open</a>
                  @if($isImage($refund->penalty_payment_proof_path))
                    <img src="{{ $fileUrl($refund->penalty_payment_proof_path) }}" class="thumb" alt="">
                  @endif
                </div>
              @endif
              <div class="hint mt-1">Keeps existing if empty</div>
            ...
            <div>
              ...
              @if($fileUrl($refund->istiraha_proof_path ?? null))
                <div class="preview mt-2">
                  <a target="_blank" href="{{ $fileUrl($refund->istiraha_proof_path) }}" class="btn btn-sm btn-outline-primary">Open</a>
                  @if($isImage($refund->istiraha_proof_path))
                    <img src="{{ $fileUrl($refund->istiraha_proof_path) }}" class="thumb" alt="">
                  @endif
                </div>
              @endif
              <div class="hint mt-1">Keeps existing if empty</div>
            ...
            <div>
              ...
              @if($fileUrl($refund->payment_proof_path ?? null))
                <div class="preview mt-2">
                  <a target="_blank" href="{{ $fileUrl($refund->payment_proof_path) }}" class="btn btn-sm btn-outline-primary">Open</a>
                  @if($isImage($refund->payment_proof_path))
                    <img src="{{ $fileUrl($refund->payment_proof_path) }}" class="thumb" alt="">
                  @endif
                </div>
              @endif
              <div class="hint mt-1">Keeps existing if empty</div>
          ...
          <div class="btns">
            ...
            <a href="{{ route('refunds.show',['refund'=>$refund->id]) }}" class="btn btn-outline-secondary">Cancel</a>
          </div>
