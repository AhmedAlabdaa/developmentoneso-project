...
<div class="sal-wrap" data-sal-root="1">
  ...
  <div class="sal-stats">
    <div class="sal-card sal-card-pending">
      ...
      <div class="sal-r">
        <div class="sal-cnt">{{ (int)($stat['pending']['count'] ?? 0) }}</div>
        <div class="sal-amt"><strong>QAR {{ number_format((float)($stat['pending']['amount'] ?? 0), 2) }}</strong></div>
      </div>
    </div>
    ...
    <div class="sal-card sal-card-paid">
      ...
      <div class="sal-r">
        <div class="sal-cnt">{{ (int)($stat['paid']['count'] ?? 0) }}</div>
        <div class="sal-amt"><strong>QAR {{ number_format((float)($stat['paid']['amount'] ?? 0), 2) }}</strong></div>
      </div>
    </div>
    ...
    <div class="sal-card sal-card-partial">
      ...
      <div class="sal-r">
        <div class="sal-cnt">{{ (int)($stat['partial_paid']['count'] ?? 0) }}</div>
        <div class="sal-amt"><strong>QAR {{ number_format((float)($stat['partial_paid']['amount'] ?? 0), 2) }}</strong></div>
      </div>
    </div>
    ...
    <div class="sal-card sal-card-cancelled">
      ...
      <div class="sal-r">
        <div class="sal-cnt">{{ (int)($stat['cancelled']['count'] ?? 0) }}</div>
        <div class="sal-amt"><strong>QAR {{ number_format((float)($stat['cancelled']['amount'] ?? 0), 2) }}</strong></div>
      </div>
    </div>
  </div>
    ...
    <table class="sal-table table-striped table-hover">
      ...
      <tbody>
        @forelse($salaries as $s)
          @php $st = $norm($s->status); @endphp
          <tr>
            <td>{{ strtoupper((string)($s->reference_no ?? '-')) }}</td>
            <td>{{ strtoupper((string)($s->passport_no ?? '-')) }}</td>
            <td>{{ strtoupper((string)($s->nationality ?? '-')) }}</td>
            <td>{{ strtoupper((string)($s->foreign_partner ?? '-')) }}</td>
            <td>{{ strtoupper((string)($s->agreement_no ?? '-')) }}</td>
            <td>{{ $s->number_of_work_days ?? $s->work_days ?? '-' }}</td>
            <td>QAR {{ number_format((float)($s->maid_salary ?? $s->total_salary ?? 0), 2) }}</td>
            <td>QAR {{ number_format((float)($s->salary_for_work_days ?? 0), 2) }}</td>
            <td>{{ strtoupper((string)($s->sales_name ?? '-')) }}</td>
            <td>
              @if($canManage)
                <select
                  class="sal-status-select {{ $cls[$st] ?? '' }}"
                  data-sal-status="1"
                  data-id="{{ $s->id }}"
                  data-original="{{ $st }}"
                  data-name="{{ strtoupper((string)($s->passport_no ?? 'EMPLOYEE')) }}"
                >
                  <option value="pending" {{ $st==='pending'?'selected':'' }}>PENDING</option>
                  <option value="paid" {{ $st==='paid'?'selected':'' }}>PAID</option>
                  <option value="partial_paid" {{ $st==='partial_paid'?'selected':'' }}>PARTIAL PAID</option>
                  <option value="cancelled" {{ $st==='cancelled'?'selected':'' }}>CANCELLED</option>
                </select>
              @else
                <span class="sal-status-text {{ $cls[$st] ?? '' }}">{{ strtoupper(str_replace('_',' ',$st)) }}</span>
              @endif
            </td>
            <td>{{ $s->created_at ? \Carbon\Carbon::parse($s->created_at)->format('d M Y h:i A') : '-' }}</td>
            <td>{{ $s->updated_at ? \Carbon\Carbon::parse($s->updated_at)->format('d M Y h:i A') : '-' }}</td>
            <td>
              <a class="sal-btn sal-btn-info" href="{{ route('salaries.show', $s->id) }}" title="View"><i class="fas fa-eye"></i></a>
              <a class="sal-btn sal-btn-warning" href="{{ route('salaries.edit', $s->id) }}" title="Edit"><i class="fas fa-pen"></i></a>
            </td>
          </tr>
        @empty
  ...
  <nav aria-label="Page navigation">
    <div class="sal-pagination">
      <span class="sal-muted">Showing {{ $salaries->firstItem() }} to {{ $salaries->lastItem() }} of {{ $salaries->total() }} results</span>
...
(function(){
  ...
  document.addEventListener('change', function(e){
    ...
    confirmBox('Change status for '+name+'?', 'Switch to "'+label+'"?').then(function(res){
      ...
      fetch("{{ route('salaries.updateStatus') }}",{
        method:'POST',
