...
<div class="remit-wrap" data-remit-root="1">
  ...
  <div class="remit-stats">
    <div class="remit-card remit-card-pending">
      ...
      <div class="remit-r">
        <div class="remit-cnt">{{ (int)($stat['pending']['count'] ?? 0) }}</div>
        <div class="remit-amt"><strong>QAR {{ number_format((float)($stat['pending']['amount'] ?? 0), 2) }}</strong></div>
      </div>
    </div>
    ...
    <div class="remit-card remit-card-paid">
      ...
      <div class="remit-r">
        <div class="remit-cnt">{{ (int)($stat['paid']['count'] ?? 0) }}</div>
        <div class="remit-amt"><strong>QAR {{ number_format((float)($stat['paid']['amount'] ?? 0), 2) }}</strong></div>
      </div>
    </div>
    ...
    <div class="remit-card remit-card-partial">
      ...
      <div class="remit-r">
        <div class="remit-cnt">{{ (int)($stat['partial_paid']['count'] ?? 0) }}</div>
        <div class="remit-amt"><strong>QAR {{ number_format((float)($stat['partial_paid']['amount'] ?? 0), 2) }}</strong></div>
      </div>
    </div>
  </div>
      ...
      <tbody>
        @forelse($remittances as $r)
          ...
          <tr>
            ...
            <td>QAR {{ number_format((float)($r->amount ?? 0), 2) }}</td>
            <td>{{ strtoupper((string)($r->sales_name ?? '-')) }}</td>
            <td>
              @if($canManage)
                <select
                  class="remit-status-select {{ $cls[$st] ?? '' }}"
                  data-remit-status="1"
                  data-id="{{ $r->id }}"
                  data-original="{{ $st }}"
              ...
              @else
                <span class="remit-status-text {{ $cls[$st] ?? '' }}">{{ strtoupper(str_replace('_',' ',$st)) }}</span>
              @endif
            </td>
            <td>{{ strtoupper((string)($r->payment_method ?? '-')) }}</td>
            ...
            <td>
              <a class="remit-btn remit-btn-info" href="{{ route('remittances.show', $r->id) }}" title="View"><i class="fas fa-eye"></i></a>
              <a class="remit-btn remit-btn-warning" href="{{ route('remittances.edit', $r->id) }}" title="Edit"><i class="fas fa-pen"></i></a>
            </td>
          </tr>
        @empty
  ...
  <nav aria-label="Page navigation">
    <div class="remit-pagination">
      <span class="remit-muted">Showing {{ $remittances->firstItem() }} to {{ $remittances->lastItem() }} of {{ $remittances->total() }} results</span>
...
(function(){
  ...
  document.addEventListener('change', function(e){
    ...
    confirmBox('Change status for '+name+'?', 'Switch to "'+label+'"?').then(function(res){
      ...
      fetch("{{ route('remittances.updateStatus') }}",{
        method:'POST',
