...
<div class="repl-wrap" data-repl-root="1">
  ...
  <div class="repl-stats">
    <div class="repl-card repl-card-open">
      ...
      <div class="repl-r">
        <div class="repl-cnt">{{ (int)($stat['open']['count'] ?? 0) }}</div>
        <div class="repl-amt"><strong>QAR {{ number_format((float)($stat['open']['amount'] ?? 0), 2) }}</strong></div>
      </div>
    </div>
    ...
    <div class="repl-card repl-card-closed">
      ...
      <div class="repl-r">
        <div class="repl-cnt">{{ (int)($stat['closed']['count'] ?? 0) }}</div>
        <div class="repl-amt"><strong>QAR {{ number_format((float)($stat['closed']['amount'] ?? 0), 2) }}</strong></div>
      </div>
    </div>
    ...
    <div class="repl-card repl-card-cancelled">
      ...
      <div class="repl-r">
        <div class="repl-cnt">{{ (int)($stat['cancelled']['count'] ?? 0) }}</div>
        <div class="repl-amt"><strong>QAR {{ number_format((float)($stat['cancelled']['amount'] ?? 0), 2) }}</strong></div>
      </div>
    </div>
  </div>
        ...
        @forelse($replacements as $r)
          ...
          <tr>
            ...
            <td>
              @if($canManage)
                <select
                  class="repl-status-select {{ $cls[$st] ?? '' }}"
                  data-repl-status="1"
                  data-id="{{ $r->id }}"
                  data-original="{{ $st }}"
              ...
              @else
                <span class="repl-status-text {{ $cls[$st] ?? '' }}">{{ strtoupper($st) }}</span>
              @endif
            </td>
            ...
            <td>
              <a class="repl-btn repl-btn-info" href="{{ route('replacements.view', $r->id) }}" title="View Replacement Form">
  ...
  <nav aria-label="Page navigation">
    <div class="repl-pagination">
      <span class="repl-muted">
        Showing {{ $replacements->firstItem() ?? 0 }} to {{ $replacements->lastItem() ?? 0 }} of {{ $replacements->total() ?? 0 }} results
      </span>
      <ul class="pagination justify-content-center">
        {{ $replacements->appends(request()->except('page'))->links('vendor.pagination.bootstrap-4') }}
      </ul>
    </div>
  </nav>
...
(function(){
  ...
  document.addEventListener('change', function(e){
    ...
    confirmBox('Change status for '+name+'?', 'Switch to "'+label+'"?').then(function(res){
      ...
      fetch("{{ route('replacements.updateStatus') }}",{
        method:'POST',
