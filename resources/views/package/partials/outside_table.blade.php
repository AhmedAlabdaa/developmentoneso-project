        ...
        @forelse ($packages as $package)
            ...
            <tr>
                <td class="align-middle">
                    <a href="" style="color:#007bff">{{ strtoupper($package->agreement_no) }}</a>
                    @php $agStatus = $package->agreement?->status; @endphp
                    <div class="text-muted small mt-1">
                        @switch($agStatus)
                            @case(1) <i class="fas fa-clock text-warning status-icon" title="Pending"></i> PENDING @break
                            @case(2) <i class="fas fa-check-circle text-success status-icon" title="Active"></i> ACTIVE @break
                            @case(3) <i class="fas fa-exclamation-circle text-warning status-icon" title="Exceeded"></i> EXCEEDED @break
                            @case(4) <i class="fas fa-times-circle text-danger status-icon" title="Rejected"></i> REJECTED @break
                            @case(5) <i class="fas fa-file-signature text-success status-icon" title="Contracted"></i> CONTRACTED @break
                            @default <i class="fas fa-question-circle text-secondary status-icon" title="Unknown"></i> UNKNOWN
                        @endswitch
                    </div>
                </td>
                ...
                <td class="align-middle text-center">
                    ...
                    <div class="text-muted small mt-1" title="{{ $package->sales_name }}">{{ strtoupper(explode(' ', $package->sales_name)[0]) }}</div>
                </td>

                <td class="align-middle">
                    <a href="{{ url('package/'.$package->id) }}" target="_blank" class="text-decoration-none" style="color:#007bff">
                        {{ $package->candidate_name ? strtoupper(\Illuminate\Support\Str::title(strtolower($package->candidate_name))) : 'N/A' }}
                    </a>

                    <img src="{{ asset('assets/img/attach.png') }}" alt="Attachment Icon" style="cursor:pointer;margin-left:8px;vertical-align:middle;height:20px;" title="View Attachments of the Candidate" onclick="showCandidateModal('{{ $package->candidate_name }}', '{{ $package->id }}', '{{ $package->reference_no }}')" />

                    <div class="text-muted small mt-1">
                        {{ strtoupper($package->passport_no) }} — {{ strtoupper($package->nationality) }} — {{ strtoupper(\Illuminate\Support\Str::before($package->foreign_partner ?? '', ' ')) }}
                    </div>
                </td>

                <td>
                    <span class="btn btn-sm btn-{{ $status['class'] }}" style="font-size:10px;color:#fff;">
                        <i class="fas fa-{{ $status['icon'] }}"></i> {{ $status['label'] }}
                    </span>
                </td>

                <td class="align-middle">
                    <a href="" class="text-decoration-none" style="color:#007bff">{{ strtoupper($package->sponsor_name) }}</a>
                    <div class="text-muted small mt-1">CLIENT #: {{ strtoupper($package->CL_Number) }} — EID: {{ strtoupper($package->eid_no) }}</div>
                </td>

                <td class="align-middle">
                    <div>{{ $package->wc_date ? \Carbon\Carbon::parse($package->wc_date)->format('d M Y') : 'N/A' }}</div>
                    <div class="text-muted small mt-1">DW #: {{ strtoupper($package->dw_number ?? 'N/A') }}</div>
                </td>

                <td class="align-middle">
                    <div>{{ $package->visa_date ? \Carbon\Carbon::parse($package->visa_date)->format('d M Y') : 'N/A' }}</div>
                    <div class="text-muted small mt-1">{{ $package->visa_type ? strtoupper($package->visa_type) : 'N/A' }}</div>
                </td>

                <td>{{ $package->arrived_date ? \Carbon\Carbon::parse($package->arrived_date)->format('d M Y') : 'N/A' }}</td>

                <td class="align-middle">
                    <div>{{ $package->incident_date ? \Carbon\Carbon::parse($package->incident_date)->format('d M Y') : 'N/A' }}</div>
                    <div class="text-muted small mt-1">{{ $package->incident_type ? strtoupper($package->incident_type) : 'N/A' }}</div>
                </td>

                <td>{{ \Carbon\Carbon::parse($package->updated_at)->format('d M Y h:i A') }}</td>

                <td>{{ strtoupper($package->remarks) }}</td>

                <td id="action-cell-{{ $package->id }}">
                    @if ($package->inside_status === 0 || $package->inside_status === 6)
                        ...
                        <div class="dropdown-container" id="dropdownContainer-{{ $package->id }}">
                            ...
                            <select class="form-control status-dropdown" onchange="confirmStatusChange(this, '{{ $package->id }}', '{{ $package->candidate_name }}')">
                                @php $allowed = [0 => 'Change Status', 5 => 'Incident']; @endphp
                                @foreach($allowed as $id => $name)
                                    <option value="{{ $id }}" @selected((int)0 === (int)$id)>{{ $name }}</option>
                                @endforeach
                            </select>
                        </div>
                    @endif
