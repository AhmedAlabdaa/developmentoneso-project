  ...
  <table class="table table-striped table-hover">
    ...
    <tbody>
      @foreach($contracts as $contract)
        ...
        <tr>
          <td><a class="link-primary" href="{{ $showContractUrl }}">{{ $contract->reference_no }}</a></td>
          <td><a class="link-primary" href="{{ $showAgreementUrl }}">{{ $contract->agreement_reference_no }}</a></td>
          ...
          <td class="text-center">
            @if($contract->contract_signed_copy)
              <button class="btn btn-success btn-icon-only view-copy" data-file="{{ $signedFile }}">
            ...
            @else
              <button
                ...
                data-reference="{{ $contract->reference_no }}"
                data-agreement="{{ $contract->agreement_reference_no }}"
                data-start="{{ $contract->contract_start_date }}"
                data-end="{{ $contract->contract_end_date }}"
                data-transferred="{{ $contract->maid_delivered ?? 'No' }}"
                data-status="{{ $contract->status ?? 1 }}"
              >
                <i class="fas fa-upload"></i>
              </button>
            @endif

            <a class="btn btn-info btn-icon-only" href="{{ $showContractUrl }}">
            ...
            <a
              ...
              onclick="openContractDropdown('{{ $contract->id }}', this, '{{ e($cnName ?: $passport ?: $contract->reference_no) }}')"
              title="Train"
            >
              <i class="fas fa-train"></i>
            </a>

            <div class="dropdown-container" id="dropdownContainer-{{ $contract->id }}">
              ...
              <div class="dropdown-header">
                ...
                <p>Change status for <span class="candidate-name">{{ $cnName ?: 'N/A' }}</span></p>
              </div>
              ...
              <select
                ...
                data-reference="{{ $contract->reference_no }}"
                data-candidate="{{ e($cnName ?: $passport ?: $contract->reference_no) }}"
                data-original=""
...
function updateContractStatus(contractId, status){
  return $.ajax({
    url: "{{ route('contracts.updateStatus') }}",
    method: "POST",
    dataType: "json",
    headers: {
