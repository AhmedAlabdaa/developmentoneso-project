    ...
    <tbody>
      @forelse($packages as $package)
        ...
        <tr>
          ...
          <td>{{ strtoupper($package->candidate_name) }}</td>
          <td>{{ strtoupper($sales->first_name.' '.$sales->last_name) }}</td>
          <td>{!! $statusBtn !!}</td>
          <td>{{ strtoupper($package->nationality) }}</td>
          <td>{{ strtoupper($package->foreign_partner) }}</td>
          <td>{{ strtoupper($client->CL_Number) }}</td>
          <td>{{ strtoupper($client->first_name.' '.$client->last_name) }}</td>
          <td>{{ strtoupper($package->visa_type) }}</td>
          <td>
            {{ $package->arrived_date
                ? \Carbon\Carbon::parse($package->arrived_date)->format('d M Y')
                : '-' }}
          </td>
          <td>{{ strtoupper($incident->incident_type ?: '-') }}</td>
          <td>
          ...
          <td class="actions">
            <a href="javascript:void(0);" class="btn btn-primary btn-icon-only"
               title="Change Status"
               onclick="openDropdown('{{ $package->id }}', this, '{{ strtoupper($package->candidate_name) }}')">
              <i class="fas fa-train"></i>
            </a>
            <div class="fullscreen-overlay" onclick="closeAllDropdowns()"></div>
            <div class="dropdown-container" id="dropdownContainer-{{ $package->id }}" style="display:none;">
              ...
              <div class="dropdown-header">
                ...
                <p><strong>{{ strtoupper($package->candidate_name) }}</strong>?</p>
              </div>
            ...
            @if(Auth::user()->role==='Admin')
              <form action="{{ route('packages.destroy',$package->id) }}"
                    method="POST" style="display:inline;" id="delete-form-{{ $package->id }}">
                @csrf @method('DELETE')
