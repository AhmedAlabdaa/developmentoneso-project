  ...
  <table class="table table-striped table-hover" id="packagesTable">
    ...
    <tbody>
      @forelse ($packages as $p)
        ...
        <tr
          data-cn="{{ strtoupper((string)($p->CN_Number ?? '')) }}"
          data-candidate="{{ e($candidateTitle) }}"
          data-nationality="{{ strtoupper((string)($p->nationality ?? '')) }}"
          data-visa="{{ strtoupper((string)($p->visa_type ?? '')) }}"
          data-expiry="{{ $expiryIso }}"
          data-expiry-human="{{ $expiryHuman }}"
          data-category="{{ strtoupper($catRaw) }}"
          data-package-id="{{ $p->id }}"
          data-passport="{{ $passportNo }}"
          data-comment-rejected="{{ $commentRejected ? 1 : 0 }}"
          data-finance-rejected="{{ $financeRejected ? 1 : 0 }}"
        >
          <td>{{ strtoupper((string)($p->CN_Number ?? '')) }}</td>
          <td>
            <div>{{ strtoupper((string)($p->candidate_name ?? '')) }}</div>
            <div class="text-muted small">
              PP: {{ $passportNo ?: 'N/A' }}
              • EXP: {{ $p->passport_expiry_date ? \Carbon\Carbon::parse($p->passport_expiry_date)->format('d M Y') : 'N/A' }}
            </div>
          </td>
          <td title="{{ $p->sales_name }}">{{ strtoupper(explode(' ', (string)$p->sales_name)[0] ?? '') }}</td>
          <td style="text-align:center;font-size:12px">
            <div>{{ $p->updated_at ? strtoupper($p->updated_at->format('d M Y')) : '' }}</div>
            <div class="mt-1">
              <span class="btn btn-sm {{ $btn }}" style="font-size:10px">
                <i class="fas {{ $icon }} mr-1"></i> {{ strtoupper($label) }}
              </span>
            </div>
          </td>
          <td style="text-align:center">
            <div class="expiry-date-text">{{ $expiryHuman }}</div>
          ...
          <td>
            <button type="button" class="btn btn-primary btn-icon-only js-view-proof" data-proof-url="{{ $p->ica_proof ? asset('storage/'.$p->ica_proof) : '' }}">
              <i class="fas fa-eye"></i>
            </button>
          </td>
          <td>{{ strtoupper((string)($p->nationality ?? '')) }}</td>
          <td title="{{ strtoupper((string)($p->foreign_partner ?? '')) }}">{{ strtoupper(strtok((string)($p->foreign_partner ?? ''), ' ')) }}</td>
          <td>{{ strtoupper((string)($p->visa_type ?? '')) }}</td>
          <td>{{ $p->arrived_date ? \Carbon\Carbon::parse($p->arrived_date)->format('d M Y') : '' }}</td>
          <td>{{ $p->returned_date ? \Carbon\Carbon::parse($p->returned_date)->format('d M Y') : '' }}</td>
          <td>{{ $p->description }}</td>
          <td>
            <div class="action-stack">
              <button type="button" class="btn btn-primary btn-icon-only js-open-status" title="Change Status"
                data-package-id="{{ $p->id }}"
                data-candidate="{{ e((string)($p->candidate_name ?? '')) }}"
                data-candidate-title="{{ e($candidateTitle) }}"
              >
              ...
              @if ($financeRejected)
                <button type="button" class="btn btn-dark btn-icon-only js-finance-rejected" title="Finance Rejected"
                  data-cn="{{ strtoupper((string)($p->CN_Number ?? '')) }}"
                  data-candidate="{{ $candidateTitle }}"
                  data-category="{{ strtoupper($catRaw) }}"
                  data-status="{{ strtoupper((string)($office->finance_status ?? '')) }}"
                  data-comments="{{ e((string)($office->finance_comments ?? '')) }}"
                  data-updated="{{ $financeUpdatedAt }}"
              ...
              <a href="{{ route('package.exit', ['reference_no' => $p->cn_number_series]) }}" class="btn btn-primary btn-icon-only" title="Exit Form" target="_blank">
                <i class="fas fa-file-export"></i>
              </a>

              <button type="button" class="btn btn-primary btn-icon-only js-edit-office" title="Edit Package"
                data-office-id="{{ $office->id ?? '' }}"
                data-candidate-id="{{ $p->id }}"
                data-cn="{{ $p->CN_Number }}"
                data-name="{{ $p->candidate_name }}"
                data-passport-no="{{ $p->passport_no }}"
                data-nationality="{{ $p->nationality }}"
                data-category="{{ $office->category ?? '' }}"
                data-returned-date="{{ $office->returned_date ?? '' }}"
                data-expiry-date="{{ $office->expiry_date ?? '' }}"
                data-ica-proof="{{ $office->ica_proof ?? '' }}"
                data-overstay-days="{{ $office->overstay_days ?? '' }}"
                data-fine-amount="{{ $office->fine_amount ?? '' }}"
                data-passport-status="{{ $office->passport_status ?? '' }}"
              >
                <i class="fas fa-edit"></i>
              </button>
            </div>

            <div class="dropdown-container" id="dropdownContainer-{{ $p->id }}">
              <div class="close-icon js-close-dropdown" data-package-id="{{ $p->id }}"><i class="fas fa-times-circle"></i></div>
              <div class="dropdown-header">
                <div class="header-icon"><i class="fas fa-info-circle"></i></div>
                <p>Do you want to change the status of</p>
                <p>package <span class="package-name" id="packageName-{{ $p->id }}"></span>?</p>
              </div>
              ...
              <select
                ...
                name="current_status"
                data-package-id="{{ $p->id }}"
                data-original="{{ $p->_status }}"
                data-cn="{{ $p->cn_number_series }}"
                data-can-convert="{{ $isNewArrival ? 1 : 0 }}"
              >
                @foreach ($allowedStatuses as $statusId => $statusName)
                  <option value="{{ $statusId }}" {{ $p->_status == $statusId ? 'selected' : '' }}>{{ $statusName }}</option>
                @endforeach
                @if ($isNewArrival)
      ...
      <form id="editOfficeForm" enctype="multipart/form-data">
        <div class="modal-body">
          ...
          <div class="row g-3">
            ...
            <div class="col-md-4 d-none">
              <input type="hidden" id="eo_updated_by" name="update_by" value="{{ auth()->id() }}">
            </div>
          </div>
        </div>
...
(() => {
  ...
  const fetchContractedDataByPassport = (passportNo, packageId) => {
    ...
    $.ajax({
      url: '{{ route("packages.office-contracted") }}',
      type: 'POST',
      headers: { 'X-CSRF-TOKEN': csrf() },
      dataType: 'json',
  ...
  $('#contractedForm').on('submit', function (e) {
    ...
    $.ajax({
      url: '{{ route("packages.office-contracted.save") }}',
      type: 'POST',
      headers: { 'X-CSRF-TOKEN': csrf() },
      data: fd,
      processData: false,
  ...
  const updateStatus = (packageId, statusId, selectEl) => {
    const url = "{{ route('packages.update-status-inside', ['packageId' => ':packageId']) }}".replace(':packageId', packageId);
    $.ajax({
      url,
      type: 'POST',
      headers: { 'X-CSRF-TOKEN': csrf() },
