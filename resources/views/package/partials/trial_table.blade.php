    ...
    <tbody>
      ...
      @else
        @foreach($packages as $trial)
          ...
          <tr style="{{ $rowStyle }}">
            <td><a href="#" target="_blank" class="text-dark text-decoration-none">{{ strtoupper($trial->agreement_reference_no ?? '') }}</a></td>
            <td>{{ strtoupper($trial->candidate_name) }}</td>
            <td>{{ strtoupper($trial->sales_name) }}</td>
            <td><button class="btn {{ $c['class'] }} btn-sm"><i class="{{ $c['icon'] }}"></i> {{ $trial->trial_status }}</button></td>
            <td>{{ $trial->created_at ? \Carbon\Carbon::parse($trial->created_at)->format('d M Y') : '-' }}</td>
            <td>{{ strtoupper($trial->nationality) }}</td>
            <td title="{{ strtoupper($trial->foreign_partner) }}">{{ strtoupper(strtok($trial->foreign_partner, ' ')) }}</td>
            <td><a href="#" class="text-dark text-decoration-none" target="_blank">{{ strtoupper($trial->first_name ?? '') }} {{ strtoupper($trial->last_name ?? '') }}</a></td>
            <td>{{ strtoupper($trial->visa_type) }}</td>
            <td>{{ $trial->arrived_date ? \Carbon\Carbon::parse($trial->arrived_date)->format('d M Y') : '-' }}</td>
            <td>{{ $trial->returned_date ? \Carbon\Carbon::parse($trial->returned_date)->format('d M Y') : '-' }}</td>
            <td>{{ $trial->expiry_date ? \Carbon\Carbon::parse($trial->expiry_date)->format('d M Y') : '-' }}</td>
            <td>{{ $trial->overstay_days }}</td>
            ...
            <td class="actions">
              @if($trial->trial_status==='Active')
                <a href="javascript:void(0)" class="btn {{ $buttonClass }} btn-sm"><i class="fas fa-clock"></i> {{ $timeDisplay }}</a>
              @endif
              <a href="{{ route('package.exit', ['reference_no' => $trial->cn_number_series]) }}" class="btn btn-primary btn-icon-only" title="Exit Form" target="_blank">
                <i class="fas fa-file-export"></i>
              </a>
              <a href="javascript:void(0)" class="btn btn-primary btn-icon-only" title="Change Status" onclick="openDropdown('{{ $trial->id }}','{{ strtoupper($trial->candidate_name) }}')">
                <i class="fas fa-train"></i>
              </a>

              <div class="dropdown-container" id="dropdownContainer-{{ $trial->id }}">
                ...
                <select class="form-control status-dropdown" onchange="trialChangeStatus(this.value, {{ $trial->trial_id }}, {{ $trial->id }}, {{ $trial->client_id }})">
                  @if($trial->trial_status==='Active')
                    <option disabled selected>Active</option>
                    <option>Contracted</option>
                    <option>Trial Return</option>
                    <option>Incident</option>
                  @elseif($trial->trial_status==='Contracted')
      ...
      <form id="ContractedModalForm" enctype="multipart/form-data" method="POST">
        ...
        <div class="modal-body">
          ...
          <div class="mb-3">
            ...
            <input type="date" id="ContractedModalDate" class="form-control" name="confirmed_date" value="{{ \Carbon\Carbon::now()->format('Y-m-d') }}">
          </div>
          <div class="row mb-3">
    ...
    <div class="modal-content">
      ...
      <form method="POST" enctype="multipart/form-data" id="ChangeStatusModalForm">@csrf
        <div class="modal-body">
          ...
          <div class="mb-3"><label for="ChangeStatusModalDate" class="form-label"><i class="fas fa-calendar-day"></i> Change Status Date</label><input type="date" id="ChangeStatusModalDate" class="form-control" name="change_status_date" value="{{ \Carbon\Carbon::now()->format('Y-m-d') }}"></div>
...
<div class="modal fade custom-modal" id="TrialReturnModal" tabindex="-1" aria-labelledby="TrialReturnModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      ...
      <form id="TrialReturnModalForm" enctype="multipart/form-data" method="POST">@csrf
...
<div class="modal fade custom-modal" id="SalesReturnModal" tabindex="-1" aria-labelledby="SalesReturnModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      ...
      <form id="SalesReturnModalForm" enctype="multipart/form-data" method="POST">@csrf
...
(function ($, w, d) {
  ...
  function updateTrialStatus(status, trialId, candidateId, clientId){
    ...
    handleFetch('{{ route("packages.updateTrialStatus") }}',{method:'POST',body:body}).then(res=>{
      if(res && res.success){
  ...
  function saveConfirmed(){
    ...
    showPreloader();
    handleFetch('{{ route("packages.saveTrialConfirmed") }}',{method:'POST',body:fd}).then(r=>{
      if(r && r.success){
  ...
  function saveChangeStatus(){
    ...
    const fd=new FormData(form);
    showPreloader();
    handleFetch('{{ route("packages.updateChangeStatus") }}',{method:'POST',body:fd}).then(dj=>{
      if(dj && dj.success){
  ...
  function saveTrialReturn(){
    ...
    const fd=new FormData(form);
    showPreloader();
    handleFetch('{{ route("packages.saveTrialReturn") }}',{method:'POST',body:fd}).then(dj=>{
      if(dj && dj.success){
  ...
  function saveSalesReturn(){
    ...
    const fd=new FormData(form);
    showPreloader();
    handleFetch('{{ route("packages.saveSalesReturn") }}',{method:'POST',body:fd}).then(dj=>{
      if(dj && dj.success){
  ...
  function saveIncident(){
    ...
    showPreloader();
    handleFetch('{{ route("packages.saveReturnIncident") }}',{method:'POST',body:fd}).then(dj=>{
      if(dj && dj.success){
