...
<div class="table-container">
  <table class="table table-striped table-hover">
    ...
    <tbody>
      @forelse($packages as $trial)
        @php
          $user    = optional($trial->createdByUser);
          $client  = optional($trial->client);
          $statusMap = [
          ...
          $st = $statusMap[$trial->trial_status] ?? ['class'=>'btn-light','icon'=>'fas fa-info-circle'];
        @endphp
        <tr>
          ...
          <td>
            <button class="btn {{ $st['class'] }} btn-sm">
              <i class="{{ $st['icon'] }}"></i> {{ $trial->trial_status }}
            </button>
          </td>
          <td>{{ strtoupper($trial->nationality) }}</td>
          <td title="{{ strtoupper($trial->foreign_partner) }}">
            {{ strtoupper(strtok($trial->foreign_partner, ' ')) }}
          </td>
          <td>
            <a href="#" class="text-dark text-decoration-none" target="_blank">
              {{ strtoupper($trial->crm_CL_Number ?? '') }}
            </a>
          </td>
          <td>
            <a href="#" class="text-dark text-decoration-none" target="_blank">
              {{ strtoupper($trial->first_name ?? '') }}  {{ strtoupper($trial->last_name ?? '') }}
            </a>
          </td>
          ...
          <td>{{ $trial->confirmed_date 
                  ? \Carbon\Carbon::parse($trial->confirmed_date)->format('d M Y') 
                  : '-' }}</td>
          <td>{{ $trial->description }}</td>
          <td class="actions">
            <a href="javascript:void(0);" class="btn btn-primary btn-icon-only" title="Change Status"
               onclick="openDropdown('{{ $trial->id }}', this, '{{ strtoupper($trial->candidate_name) }}')">
            ...
            <div class="dropdown-container" id="dropdownContainer-{{ $trial->id }}" style="display:none;">
              ...
              <div class="dropdown-header">
                ...
                <p><strong>{{ strtoupper($trial->candidate_name) }}</strong>?</p>
              </div>
              <select class="form-control status-dropdown" 
                      onchange="trialChangeStatus(this, {{ $trial->trial_id }}, {{ $trial->id }}, {{ $trial->client_id }})">
...
<script>
  ...
  function updateTrialStatus(status, trialId, candidateId, clientId){
    ...
    const updateStatusUrl = '{{ route("packages.updateTrialStatus") }}';
    showPreloader();
    const ajaxData = {
      trial_id: trialId,
  ...
  function fillModalFields(modalId, data, trialId) {
   ...
   $('#saveConfirmedModalButton').on('click', function() {
      ...
      $.ajax({
        url: '{{ route("packages.saveTrialConfirmed") }}',
        type: 'POST',
        headers: { 'X-CSRF-TOKEN': csrfToken },
        data: formData,
        processData: false,
  ...
  document.getElementById('saveChangeStatusModalButton').addEventListener('click', function(){
    ...
    fetch('{{ route("packages.updateChangeStatus") }}', {
      method: 'POST',
      headers: { 'X-CSRF-TOKEN': csrfToken },
      body: formData
    })
  ...
  document.getElementById('saveTrialReturnButton').addEventListener('click', function(){
    ...
    const csrfToken = '{{ csrf_token() }}';
    button.disabled = true;
    fetch('{{ route("packages.saveTrialReturn") }}', {
      method: 'POST',
      headers: { 'X-CSRF-TOKEN': csrfToken },
      body: formData
    })
  ...
  document.getElementById('saveSalesReturnButton').addEventListener('click', function(){
    ...
    const csrfToken = '{{ csrf_token() }}';
    button.disabled = true;
    fetch('{{ route("packages.saveSalesReturn") }}', {
      method: 'POST',
      headers: { 'X-CSRF-TOKEN': csrfToken },
      body: formData
    })
  ...
  document.getElementById('saveIncidentButton').addEventListener('click', function(){
    ...
    const csrfToken = '{{ csrf_token() }}';
    button.disabled = true;
    fetch('{{ route("packages.saveReturnIncident") }}', {
      method: 'POST',
      headers: { 'X-CSRF-TOKEN': csrfToken },
      body: formData
    })
  ...
  function openIncidentModal(trialId, candidateId, clientId) {
    ...
    $.ajax({
      url: "{{ route('package.incidentData', ':id') }}".replace(':id', trialId),
      type: "GET",
      headers: { 'X-CSRF-TOKEN': csrfToken },
      success: function(response) {
        $('#incident_candidate_id').val(candidateId);
