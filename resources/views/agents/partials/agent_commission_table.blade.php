    ...
    <table class="table sticky-table table-striped table-hover">
        ...
        <tbody>
            @forelse ($commissions as $commission)
                <tr>
                    <td>{{ $loop->iteration + ($commissions->currentPage() - 1) * $commissions->perPage() }}</td> <!-- Calculate SR # -->
                    <td>{{ $commission->agent_name ?? 'N/A' }}</td>
                    <td>{{ $commission->cl_number ?? 'N/A' }}</td>
                    <td>{{ $commission->cn_number ?? 'N/A' }}</td>
                    <td>{{ number_format($commission->credit_amount ?? 0, 2) }}</td>
                    <td>{{ number_format($commission->debit_amount ?? 0, 2) }}</td>
                    <td>{{ ucfirst($commission->status ?? 'N/A') }}</td>
                    <td>{{ $commission->incident_no ?? 'N/A' }}</td>
                    <td>{{ $commission->incident_proof ?? 'N/A' }}</td>
