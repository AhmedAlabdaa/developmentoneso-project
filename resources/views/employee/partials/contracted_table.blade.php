...
<body>
            ...
            <tr>
              <td>
                @if ($contract->reference_no)
                  <a class="link-primary" href="{{ route('contracts.show', $contract->reference_no) }}">{{ $contract->reference_no }}</a>
                @else
                  —
                @endif
              </td>
              <td>{{ $contract->created_at ? \Carbon\Carbon::parse($contract->created_at)->format($fmt) : '—' }}</td>
              <td title="{{ $salesFull }}">{{ $salesFull }}</td>
              <td><button class="btn btn-sm {{ $ct['class'] }}"><i class="{{ $ct['icon'] }}"></i> {{ $ct['label'] }}</button></td>
              <td><button class="btn btn-sm {{ $finance['class'] }}"><i class="{{ $finance['icon'] }}"></i> {{ $finance['label'] }}</button></td>
              <td>{{ $invoiceNo }}</td>
              <td>{{ $invoiceDateRaw ? \Carbon\Carbon::parse($invoiceDateRaw)->format($fmt) : '—' }}</td>
              <td>{{ $cnNumber }}</td>
              ...
              <td>{{ $money($monthlyAmount) }}</td>
              <td>{{ $paidDateRaw ? \Carbon\Carbon::parse($paidDateRaw)->format($fmt) : '—' }}</td>
              <td>{{ $paymentMode }}</td>
              <td>{{ $upcomingPaymentRaw ? \Carbon\Carbon::parse($upcomingPaymentRaw)->format($fmt) : '—' }}</td>
              <td>
                <button
                  type="button"
                  class="btn btn-sm {{ $instBtnClass }} {{ $blink ? 'btn-blink' : '' }} open-contract-installments"
                  data-contract-ref="{{ $contract->reference_no }}"
                  data-agreement-ref="{{ $contract->agreement_reference_no }}"
                  data-total="{{ $totalItems }}"
                  data-paid="{{ $paidItems }}"
                  {{ $disabled ? 'disabled' : '' }}
                >
                  <i class="{{ $instIcon }} me-1"></i>{{ $instLabel }}
                </button>
              </td>
              <td>{{ $cli?->CL_Number ?? '—' }}</td>
              <td>{{ $agr?->candidate_name ?? '—' }}</td>
              <td>{{ $agr?->nationality ?? '—' }}</td>
              <td>{{ $customerName }}</td>
              <td>{{ $money($contractedAmount) }}</td>
              <td>{{ $money($receivedAmount) }}</td>
              <td>{{ $money($balanceAmount) }}</td>
              <td>{{ $money($maidSalary) }}</td>
              ...
              <td class="text-center">
                <button
                  ...
                  data-cn="{{ $agr?->CN_Number ?? '' }}"
                  data-ref="{{ $agr?->reference_no ?? '' }}"
                  data-contract-ref="{{ $contract->reference_no }}"
                  data-id="{{ $contract->id }}"
                  data-current="{{ $contract->marked ?? '' }}"
  ...
  <div class="modal fade custom-modal" id="UpdateContractModal" tabindex="-1">
    <div class="modal-dialog modal-lg">
      @if(auth()->check() && in_array(auth()->user()->role, ['Admin', 'Managing Director','Sales Officer']))
  ...
  <script>
    (function(){
      ...
      window.showOffice=function(passportNo){
        ...
        $.post(`{{ route('employees.update-status-inside',['employeeId'=>':id']) }}`.replace(':id',encodeURIComponent(passportNo)),{status_id:1,_token:csrf}).done(res=>{
      ...
      window.showIncident=function(passportNo){
        ...
        $.post(`{{ route('employees.update-status-inside',['employeeId'=>':id']) }}`.replace(':id',encodeURIComponent(passportNo)),{status_id:3,_token:csrf}).done(res=>{
          let d=res.candidateDetails;
      ...
      function openUploadModal(ref){
        $.post('{{ route("contracts.detailsAll") }}',{agreement_reference_no:ref,_token:csrf}).done(d=>{
      ...
      function showContractInstallments(ref,agreementNo){
        ...
        $m.modal('show');
        $.post('{{ route("contracts.installments.items") }}',{reference_no:ref,agreement_reference_no:agreementNo,_token:csrf})
          .done(items=>{
      ...
      $(function(){
        ...
        $('#replacementForm').on('submit',function(e){
          ...
          let url=`{{ url('contracts') }}/${ref}/replace`;
        ...
        $('#saveOfficeBtnEmp').on('click',function(){
          ...
          $.ajax({url:'{{ route("employees.officeSave") }}',method:'POST',data:fd,processData:false,contentType:false}).done(r=>{
        ...
        $('#saveIncidentBtn').on('click',function(){
          ...
          $.ajax({url:'{{ route("employees.incidentSave") }}',method:'POST',data:fd,processData:false,contentType:false}).done(r=>{
            toastr.success(r.message);
            setTimeout(()=>location.reload(),1000);
          }).fail(e=>{
        ...
        $('#saveUpdateContract').click(function(){
          ...
          $.ajax({url:'{{ route("contracts.update-contract") }}',method:'POST',data:fd,processData:false,contentType:false}).done(res=>{
        ...
        $(document).on('click','.generate-invoice-btn',async function(){
          ...
          try{
            const r=await fetch('{{ route('installments.generate-invoice') }}',{method:'POST',headers:{'X-CSRF-TOKEN':csrf,'Accept':'application/json','Content-Type':'application/json'},body:JSON.stringify({id:btn.dataset.id})});
        ...
        $popover.find('.act-replacement').on('click',function(){
          if(!actionCtx)return;
          closePopover();
          $.post('{{ route("contracts.detailsAll") }}',{agreement_reference_no:actionCtx.ref,_token:csrf}).done(d=>{
        ...
        $popover.find('.act-mark').on('click',function(){
          ...
          Swal.fire({title:`Mark as ${next}?`,icon:'question',showCancelButton:true,confirmButtonText:'Yes',cancelButtonText:'No'}).then(r=>{
            if(!r.isConfirmed)return;
            $.post('{{ route("contracts.toggleMarked") }}',{id:actionCtx.id,reference_no:actionCtx.contractRef,marked:next,_token:csrf}).done(res=>{
              toastr.success(res.message);
