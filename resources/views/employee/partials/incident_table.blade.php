  ...
  <table class="table table-striped table-hover">
    ...
    <tbody>
      @forelse($employees as $emp)
        <tr>
          <td><a href="{{ route('employees.show', $emp->reference_no) }}">{{ $emp->reference_no }}</a></td>
          <td>{{ $emp->incident_date ? \Carbon\Carbon::parse($emp->incident_date)->format('d M Y') : 'N/A' }}</td>
          <td><a href="{{ route('employees.show', $emp->reference_no) }}">{{ $emp->name }}</a></td>
          <td>{{ $emp->passport_no }}</td>
          <td>{{ $emp->nationality }}</td>
          <td>{{ $emp->incident_type }}</td>
          <td>{{ $emp->package }}</td>
          <td>{{ $emp->visa_status }}</td>
          <td>{{ $emp->foreign_partner }}</td>
          <td>
            <button type="button"
                    class="btn btn-primary btn-sm btn-update-incident"
                    data-ref="{{ $emp->reference_no }}"
                    data-name="{{ $emp->name }}"
                    data-passport="{{ $emp->passport_no }}"
                    data-date="{{ $emp->incident_date }}"
                    data-type="{{ $emp->incident_type }}"
                    data-comments="{{ $emp->comments ?? '' }}">
...
<nav class="pagination-controls">
  <span>Showing {{ $employees->firstItem() }} to {{ $employees->lastItem() }} of {{ $employees->total() }} results</span>
...
<script>
  ...
  $('#saveUpdateIncident').on('click', function() {
    $.post('{{ route("employees.incidentSave") }}', $('#updateIncidentForm').serialize(), function() {
