    ...
    <tbody>
      ...
      @else
        @foreach ($trials as $trial)
          ...
          <tr style="{{ $rowClass }}">
            <td>
              @if ($trial->agreement_reference_no)
                <a href="{{ route('agreements.show', $trial->agreement_reference_no) }}" target="_blank" class="text-decoration-none text-dark">
                  {{ strtoupper($trial->agreement_reference_no) }}
                </a>
              @else
                N/A
              @endif
            </td>
            <td>{{ \Carbon\Carbon::parse($trial->updated_at)->format('d M Y h:i A') }}</td>
            <td>
              <a href="{{ route($trial->trial_type . 's.show', $trial->reference_no) }}" target="_blank" class="text-decoration-none text-dark">
                {{ strtoupper($trial->CN_Number) }}
              </a>
            </td>
            <td>
              <a href="{{ route('crm.show', $trial->client->slug) }}" target="_blank" class="text-decoration-none text-dark">
                {{ strtoupper($trial->client->CL_Number) }}
              </a>
            </td>
            <td>{{ strtoupper($trial->candidate_name) }}</td>
            <td>{{ strtoupper(optional($trial->createdByUser)->first_name . ' ' . optional($trial->createdByUser)->last_name) ?: 'N/A' }}</td>
            <td>
              <div>{{ strtoupper(\Carbon\Carbon::parse($trial->updated_at)->timezone('Asia/Dubai')->format('d-M-Y')) }}</div>
              <div>{{ strtoupper(\Carbon\Carbon::parse($trial->updated_at)->timezone('Asia/Dubai')->format('h:i A')) }}</div>
            </td>
            <td>{{ \Carbon\Carbon::parse($trial->trial_start_date)->format('d M Y') }}</td>
            <td>{{ \Carbon\Carbon::parse($trial->trial_end_date)->format('d M Y') }}</td>
            <td>{{ $trial->number_of_days }}</td>
            @php
              $statusConfig = [
            ...
            <td>
              <button class="btn {{ $statusClass }} btn-sm">
                <i class="{{ $statusIcon }}"></i> {{ $trial->trial_status }}
              </button>
            </td>
            <td class="actions">
              @if ($trial->trial_status === "Active")
                <a href="#" class="btn {{ $buttonClass }} btn-sm">
                  <i class="fas fa-clock"></i> {{ $timeDisplay }}
                </a>
              @endif
              <a href="javascript:void(0);" class="btn btn-primary btn-icon-only" title="Change Status" onclick="openDropdown('{{ $trial->id }}', this, '{{ strtoupper($trial->candidate_name) }}')">
                <i class="fas fa-train"></i>
              </a>
              <div class="fullscreen-overlay" id="fullscreenOverlay" onclick="closeAllDropdowns()"></div>
              <div class="dropdown-container" id="dropdownContainer-{{ $trial->id }}" style="display: none;">
                ...
                <div class="dropdown-header">
                  ...
                  <p>candidate <span id="candidateName-{{ $trial->id }}" class="candidate-name">{{ strtoupper($trial->candidate_name) }}</span>?</p>
                </div>
                <select class="form-control status-dropdown" id="statusDropdown-{{ $trial->id }}" name="current_status" onchange="trialChangeStatus(this, {{ $trial->id }}, {{ $trial->candidate_id }}, {{ $trial->client_id }})">
                  @if ($trial->trial_status === "Active")
                  ...
                  @else
                    <option value="{{ $trial->trial_status }}">{{ $trial->trial_status }}</option>
                  @endif
                </select>
              </div>
              <a href="{{ route($trial->trial_type . '.exit', ['reference_no' => $trial->reference_no]) }}"
              ...
              @if (Auth::user()->role === 'Admin')
                <form action="{{ route('employees.destroy', $trial->reference_no) }}"
                      method="POST"
                      style="display:inline;"
                      id="delete-form-{{ $trial->reference_no }}">
                  @csrf
                  @method('DELETE')
                  <button type="button"
                          class="btn btn-danger btn-icon-only"
                          onclick="confirmDelete('{{ $trial->reference_no }}')"
                          title="Delete">
...
<nav class="py-2">
  <div class="d-flex justify-content-between align-items-center">
    <span class="text-muted small">Showing {{ $trials->firstItem() }} to {{ $trials->lastItem() }} of {{ $trials->total() }} results</span>
    <ul class="pagination mb-0">{{ $trials->appends(request()->except('page'))->links('vendor.pagination.bootstrap-4') }}</ul>
  </div>
</nav>
      ...
      <form id="ConfirmedModalForm" enctype="multipart/form-data" method="POST">
        ...
        <div class="modal-body">
          ...
          <div class="mb-3">
            ...
            <input type="date" id="ConfirmedModalDate" class="form-control" name="confirmed_date" value="{{ \Carbon\Carbon::now()->format('Y-m-d') }}">
          </div>
          <div class="mb-3">
      ...
      <form method="POST" enctype="multipart/form-data" id="ChangeStatusModalForm">
        ...
        <div class="modal-body">
          ...
          <div class="mb-3">
            ...
            <input type="date" id="ChangeStatusModalDate" class="form-control" name="change_status_date" value="{{ \Carbon\Carbon::now()->format('Y-m-d') }}">
          </div>
          <div class="mb-3">
...
<script>
  ...
  function updateTrialStatus(status, trialId, candidateId, clientId){
    ...
    const updateStatusUrl = '{{ route("employees.updateTrialStatus") }}';
    showPreloader();
    const ajaxData = {
      trial_id: trialId,
  ...
  function fillModalFields(modalId, data, trialId) {
   ...
   $('#saveConfirmedModalButton').on('click', function() {
      ...
      $.ajax({
        url: '{{ route("employees.saveTrialConfirmed") }}',
        type: 'POST',
        headers: { 'X-CSRF-TOKEN': csrfToken },
        data: formData,
        processData: false,
  ...
  document.getElementById('saveChangeStatusModalButton').addEventListener('click', function(){
    ...
    fetch('{{ route("employees.updateChangeStatus") }}', {
      method: 'POST',
      headers: { 'X-CSRF-TOKEN': csrfToken },
      body: formData
    })
  ...
  document.getElementById('saveTrialReturnButton').addEventListener('click', function(){
    ...
    const csrfToken = '{{ csrf_token() }}';
    button.disabled = true;
    fetch('{{ route("employees.saveTrialReturn") }}', {
      method: 'POST',
      headers: { 'X-CSRF-TOKEN': csrfToken },
      body: formData
    })
  ...
  document.getElementById('saveSalesReturnButton').addEventListener('click', function(){
    ...
    const csrfToken = '{{ csrf_token() }}';
    button.disabled = true;
    fetch('{{ route("employees.saveSalesReturn") }}', {
      method: 'POST',
      headers: { 'X-CSRF-TOKEN': csrfToken },
      body: formData
    })
  ...
  document.getElementById('saveIncidentButton').addEventListener('click', function(){
    ...
    const csrfToken = '{{ csrf_token() }}';
    button.disabled = true;
    fetch('{{ route("employees.saveReturnIncident") }}', {
      method: 'POST',
      headers: { 'X-CSRF-TOKEN': csrfToken },
      body: formData
    })
