...
<body>
    ...
    <table class="table table-striped table-hover">
      ...
      <tbody>
        ...
        >
          ...
          <td>{{ $e->nationality }}</td>
          <td>{{ $e->category }}</td>
          <td>{{ $e->arrived_date ? Carbon::parse($e->arrived_date)->format('d M Y') : '–' }}</td>
          <td>
            @if($e->visa_status==0)
            ...
            @else
              ...
              @endphp
              <button class="btn btn-info btn-sm"><i class="fas {{ $ico }}"></i> {{ $lbl }}</button>
            @endif
          </td>
          <td>{{ $e->returned_date ? Carbon::parse($e->returned_date)->format('d M Y') : '–' }}</td>
          <td>{{ $e->expiry_date ? Carbon::parse($e->expiry_date)->format('d M Y') : '–' }}</td>
          <td>
            <button class="btn btn-primary btn-icon-only btn-view-proof" data-proof-url="{{ $e->ica_proof ? asset('storage/'.$e->ica_proof) : '' }}">
              <i class="fas fa-eye"></i>
            </button>
          </td>
          <td>{{ $overstayDays }}</td>
          <td>{{ number_format($totalFine, 2) }}</td>
          <td>{{ $e->passport_status }}</td>
          <td>
            <button
              class="btn btn-primary btn-icon-only btn-edit-office"
              data-office-id="{{ optional($e->office)->id }}"
              data-candidate-id="{{ $e->id }}"
              data-name="{{ $e->name }}"
              data-passport-no="{{ $e->passport_no }}"
              data-nationality="{{ $e->nationality }}"
              data-category="{{ optional($e->office)->category }}"
              data-returned-date="{{ optional($e->office)->returned_date ? Carbon::parse(optional($e->office)->returned_date)->format('Y-m-d') : '' }}"
              data-expiry-date="{{ optional($e->office)->expiry_date ? Carbon::parse(optional($e->office)->expiry_date)->format('Y-m-d') : '' }}"
              data-ica-proof="{{ optional($e->office)->ica_proof }}"
              data-passport-status="{{ optional($e->office)->passport_status ?: $e->passport_status }}"
            >
              <i class="fas fa-edit"></i>
            </button>

            <button class="btn btn-primary btn-icon-only btn-contract" data-passport="{{ $e->passport_no }}">
              <i class="fas fa-file-contract"></i>
            </button>

            <button class="btn btn-danger btn-icon-only btn-incident" data-passport="{{ $e->passport_no }}">
              <i class="fas fa-exclamation-triangle"></i>
            </button>

            @if(in_array($e->inside_status, [1]))
              <a href="{{ route('employee.exit', $e->passport_no) }}" class="btn btn-warning btn-icon-only">
  ...
  <nav class="py-2">
    <div class="d-flex justify-content-between align-items-center">
      <span class="text-muted small">Showing {{ $employees->firstItem() }}–{{ $employees->lastItem() }} of {{ $employees->total() }} results</span>
          ...
          <div class="form-group">
            ...
            <div class="input-group">
              ...
              <select name="months_count" id="contractMonthsCount" class="form-select">
                ...
                @for($i=2; $i<=24; $i++)<option value="{{ $i }}">{{ $i }} Months</option>@endfor
              </select>
            </div>
          </div>
          <div class="form-group">
            ...
            <select name="payment_cycle" id="contractCycle" class="form-select" required>
              ...
              @for($i=1; $i<=12; $i++)<option value="{{ $i }}">{{ $i }} Month{{ $i>1?'s':'' }}</option>@endfor
            </select>
          </div>
        </div>
    ...
    <div class="modal-card">
      ...
      <form id="editOfficeForm" enctype="multipart/form-data">
        <div class="modal-body">
          ...
          <input type="hidden" id="eo_update_by" name="update_by" value="{{ auth()->id() }}">
          <div class="form-row">
            <div class="form-group">
  ...
  <script>
    $(function () {
      ...
      $('#saveContract').on('click', function () {
        ...
        $.ajax({
          url: '{{ route("agreements.insideempagreement") }}',
          method: 'POST',
          data,
          processData: false,
          contentType: false
      ...
      $('#saveIncident').on('click', function () {
        $.post('{{ route("employees.incidentSave") }}', $('#incident_form').serialize())
      ...
      $('#eo_save_btn').on('click', function () {
        ...
        $.ajax({
          url: "{{ route('employee.office.save') }}",
          type: 'POST',
