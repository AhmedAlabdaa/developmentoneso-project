    ...
    <tbody>
      @forelse($employees as $e)
        ...
        <tr class="employee-row"
            data-name="{{ e($e->name) }}"
            data-nationality="{{ e($e->nationality) }}"
            data-passport="{{ e($e->passport_no) }}"
            data-expiry="{{ $expiryISO }}">
          <td>
            <a href="{{ route('employees.show',$e->reference_no) }}" style="color:#007bff">
              {{ $e->reference_no }}
            </a>
          </td>
          ...
          <td>
            <button type="button" class="btn btn-{{ $from['class'] }} btn-sm" style="font-size:10px;color:#fff">
              <i class="fas fa-{{ $from['icon'] }}"></i> {{ strtoupper($from['label']) }}
            </button>
          </td>
          ...
          <td>
            <div class="pill-row" style="justify-content:flex-start">
              <span class="pill {{ $expiryPill }}">
                <i class="fas {{ $expiryIcon }}"></i>
                <span>{{ $expiryHuman }}</span>
              </span>
            </div>
          </td>
          ...
          <td>
            <div class="pill-row">
              <span class="pill {{ $expiredDaysPill }}">
                <i class="fas fa-stopwatch"></i>
                <span>Expired Days: <span class="money">{{ (int)$expiredDays }}</span></span>
              </span>
              <span class="pill {{ $finePill }}">
                <i class="fas fa-money-bill-wave"></i>
                <span>Fine: <span class="money">{{ (int)$fine }}</span> AED</span>
              </span>
            </div>
          </td>

          @if((int)$e->inside_status === 0)
            <td>
              <button class="btn btn-sm btn-secondary status-button" onclick="updateStatus('{{ $e->passport_no }}',1)">
                <i class="fas fa-circle-notch"></i> No Status
              </button>
            </td>
          @else
            <td>
              <button class="btn btn-sm btn-{{ $st['class'] }}">
          ...
          <td>
            ...
            <img src="{{ asset('assets/img/attach.png') }}" alt="Attachment Icon" style="cursor:pointer;margin-left:8px;vertical-align:middle;height:20px;" title="View Attachments of the Candidate" onclick="showCandidateModal('{{ $e->name }}','{{ $e->id }}','{{ $e->reference_no }}')"/>
          </td>

          <td>{{ $e->passport_no }}</td>
          <td>{{ $e->nationality }}</td>
          <td>{{ $shortPartner($e->foreign_partner) }}</td>
          <td>{{ $e->package }}</td>
          <td>{{ $e->passport_expiry_date ? \Carbon\Carbon::parse($e->passport_expiry_date)->format('d M Y') : 'N/A' }}</td>
          <td>{{ $e->passport_issue_date ? \Carbon\Carbon::parse($e->passport_issue_date)->format('d M Y') : 'N/A' }}</td>
          <td>{{ $e->date_of_joining ? \Carbon\Carbon::parse($e->date_of_joining)->format('d M Y') : 'N/A' }}</td>
          <td>{{ $e->visa_designation }}</td>
          <td>{{ $e->date_of_birth ? \Carbon\Carbon::parse($e->date_of_birth)->format('d M Y') : 'N/A' }}</td>
          <td>{{ $e->gender }}</td>
          <td>{{ $e->employment_contract_start_date ? \Carbon\Carbon::parse($e->employment_contract_start_date)->format('d M Y') : 'N/A' }}</td>
          <td>{{ $e->employment_contract_end_date ? \Carbon\Carbon::parse($e->employment_contract_end_date)->format('d M Y') : 'N/A' }}</td>
          <td>{{ $e->contract_type }}</td>
          <td>{{ $e->salary_as_per_contract }}</td>
          <td>{{ $e->basic }}</td>
          <td>{{ $e->housing }}</td>
          <td>{{ $e->transport }}</td>
          <td>{{ $e->other_allowances }}</td>
          ...
          <td>
            <button class="btn btn-info btn-icon-only" onclick="viewProof('{{ $e->ica_proof ? asset('storage/'.$e->ica_proof) : '' }}')">
              <i class="fas fa-eye"></i>
            </button>
            <a href="{{ route('employees.edit',$e->reference_no) }}" class="btn btn-primary btn-icon-only">
              <i class="fas fa-pencil"></i>
            </a>
            @if(in_array($e->inside_status, [1]))
              <a href="{{ route('employee.exit',$e->reference_no) }}" class="btn btn-warning btn-icon-only">
                <i class="fas fa-sign-out-alt"></i>
              </a>
            @endif
            <a href="{{ route('employee.showCV',['employee'=>$e->reference_no]) }}" target="_blank" class="btn btn-info btn-icon-only ms-1" title="View CV">
...
<nav class="py-2">
  <div class="d-flex justify-content-between align-items-center">
    <span class="text-muted small">Showing {{ $employees->firstItem() }}–{{ $employees->lastItem() }} of {{ $employees->total() }}</span>
    <ul class="pagination mb-0">{{ $employees->appends(request()->except('page'))->links('vendor.pagination.bootstrap-4') }}</ul>
  </div>
</nav>
      ...
      <div class="modal-header modal-header-gradient">
        <h5 class="modal-title">
          ...
          <span class="overstay-badge ms-2">
            Fine rate: <span class="money">{{ number_format((float)$FINE_PER_DAY_AED,2) }} AED</span> / day
          </span>
        </h5>
...
(function(w,$){
  ...
  if(typeof w.FINE_PER_DAY_AED==='undefined'||w.FINE_PER_DAY_AED===null) w.FINE_PER_DAY_AED=Number({{ (float)$FINE_PER_DAY_AED }});
  ...
  w.updateStatus=function(passportNo,status){
    var url=`{{ route('employees.update-status-inside',['employeeId'=>':id']) }}`.replace(':id',encodeURIComponent(String(passportNo||'')));
    $.post(url,{status_id:status,_token:'{{ csrf_token() }}'})
      .done(function(res){
  ...
  function saveOfficeData(){
    ...
    $.ajax({url:'{{ route("employees.officeSave") }}',method:'POST',data:fd,processData:false,contentType:false})
      .done(function(res){
