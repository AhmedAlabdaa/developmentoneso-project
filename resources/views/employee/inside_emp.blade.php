...
<main id="main" class="main">
  <section class="section" data-package="{{ $activePkg }}">
    ...
    <div class="row mb-2">
      ...
      <div class="col-lg-8 d-flex justify-content-end align-items-center position-relative">
        ...
        @if(auth()->user()->role === 'Archive Clerk' || auth()->user()->role === 'Admin')
          <a href="{{ route('employees.create') }}?package={{ $activePkg }}" class="btn btn-primary btn-sm"><i class="fas fa-plus"></i></a>
        @endif
      </div>
    </div>
    ...
    <div class="row">
      <div class="col-lg-12">
        <ul class="nav nav-tabs" id="statusTab">
          @foreach($tabsFirstTen as $tab => $label)
            <li class="nav-item">
              <button class="nav-link {{ $loop->first ? 'active' : '' }}" data-tab="{{ $tab }}" onclick="switchTab('{{ $tab }}', this)">
                <i class="bi bi-{{ $tabIcons[$tab] ?? 'circle' }}"></i>
                {{ $label }}
                <span id="badge_count_{{ $tab }}" class="badge bg-info">{{ $counts[$tab] ?? 0 }}</span>
              </button>
            </li>
          @endforeach

          @if(count($tabsExtra) > 0)
            ...
            @foreach($tabsExtra as $tab => $label)
              <li class="nav-item extra-tab d-none">
                <button class="nav-link" data-tab="{{ $tab }}" onclick="switchTab('{{ $tab }}', this)">
                  <i class="bi bi-{{ $tabIcons[$tab] ?? 'circle' }}"></i>
                  {{ $label }}
                  <span id="badge_count_{{ $tab }}" class="badge bg-info">{{ $counts[$tab] ?? 0 }}</span>
                </button>
      ...
      <div class="col-lg-12">
        <div id="filterPanel" class="card p-3 shadow-sm custom-panel">
          ...
          <form id="filterForm" class="row g-2">
            <input type="hidden" id="base_package" value="{{ $activePkg }}">
    ...
    <div class="table-responsive">
      @foreach(array_keys($tabsConfig) as $tab)
        <div id="table_{{ $tab }}" style="{{ $loop->first ? '' : 'display:none' }}"></div>
      @endforeach
    </div>
  </section>
</main>
...
(function(){
  ...
  const basePackage=String((sectionEl&&sectionEl.getAttribute('data-package'))||'{{ $activePkg }}'||'PKG-2').trim()||'PKG-2';

  const routes={
    all:"{{ route('employees.inside') }}",
    office:"{{ route('employees.office') }}",
    trial:"{{ route('employees.contracted') }}",
    invoices:"{{ route('employees.emp-invoices') }}",
    receipt:"{{ route('employees.emp-receipts') }}",
    ins_invoices:"{{ route('employees.emp-ins-invoices') }}",
    refund:"{{ route('employees.emp-refunds') }}",
    payroll:"{{ route('employees.emp-payroll') }}",
    incident:"{{ route('employees.incident') }}",
    outside:"{{ route('employees.outside') }}",
    boa:"{{ route('employees.emp-boa') }}",
    rvo:"{{ route('employees.emp-rvo') }}",
    replacements:"{{ route('employees.getReplaced') }}"
  };
...
window.confirmInvoiceStatusChangeofEmployees = function(el, id, name, amt, agr){
  ...
  const proceed = () => {
    ...
    const url = String(@json(route('invoices.updateStatus', ':id'))).replace(':id', id);
