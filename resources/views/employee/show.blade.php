        ...
        <div class="emp-head">
          <div class="emp-title">
            ...
            <div class="emp-badges">
              <span class="badge-pill {{ $badgeClass($employee->inside_status ?? null) }}">
                Inside Status: {{ $insideStatusLabel($employee->inside_status ?? null) }}
              </span>
              <span class="badge-pill">
                Outside Status: {{ $countryLabel($employee->inside_country_or_outside ?? null) }}
              </span>
              <span class="badge-pill">
                Reference: {{ $val($employee->reference_no ?? null) }}
              </span>
              <span class="badge-pill">
                Passport: {{ $val($employee->passport_no ?? null) }}
              </span>
            </div>
          </div>
          <div class="emp-sub">
            <i class="fa-regular fa-clock"></i> {{ $now }}
          </div>
        </div>
        ...
        <div class="emp-body">
          <div class="grid">
            <div class="box" style="grid-column: span 12;">
              ...
              <div class="kv">
                <div class="row"><div class="k">Name</div><div class="v">{{ $val($employee->name ?? null) }}</div></div>
                <div class="row"><div class="k">Nationality</div><div class="v">{{ $val($employee->nationality ?? null) }}</div></div>
                <div class="row"><div class="k">Gender</div><div class="v">{{ $val($employee->gender ?? null) }}</div></div>
                <div class="row"><div class="k">Date of Birth</div><div class="v">{{ $fmt($employee->date_of_birth ?? null) }}</div></div>
                <div class="row"><div class="k">Package</div><div class="v">{{ $val($employee->package ?? null) }}</div></div>
                <div class="row"><div class="k">Foreign Partner</div><div class="v">{{ $val($employee->foreign_partner ?? null) }}</div></div>
                <div class="row"><div class="k">Passport Expiry</div><div class="v">{{ $fmt($employee->passport_expiry_date ?? null) }}</div></div>
                <div class="row"><div class="k">Date of Joining</div><div class="v">{{ $fmt($employee->date_of_joining ?? null) }}</div></div>
              </div>
            </div>
            ...
            <div class="box" style="grid-column: span 6;">
              <h6>Visa / IDs</h6>
              <div class="kv one">
                <div class="row"><div class="k">UID No</div><div class="v">{{ $val($employee->uid_no ?? $employee->uid_number ?? null) }}</div></div>
                <div class="row"><div class="k">Personal No</div><div class="v">{{ $val($employee->personal_no ?? null) }}</div></div>
                <div class="row"><div class="k">Entry Permit No</div><div class="v">{{ $val($employee->file_entry_permit_no ?? null) }}</div></div>
                <div class="row"><div class="k">Entry Permit Issued</div><div class="v">{{ $fmt($employee->file_entry_permit_issued_date ?? null) }}</div></div>
                <div class="row"><div class="k">Entry Permit Expiry</div><div class="v">{{ $fmt($employee->file_entry_permit_expired_date ?? null) }}</div></div>
                <div class="row"><div class="k">Emirates ID</div><div class="v">{{ $val($employee->emirates_id_number ?? null) }}</div></div>
                <div class="row"><div class="k">EID Issued</div><div class="v">{{ $fmt($employee->eid_issued_date ?? null) }}</div></div>
                <div class="row"><div class="k">EID Expiry</div><div class="v">{{ $fmt($employee->eid_expiry_date ?? null) }}</div></div>
              </div>
            </div>
            ...
            <div class="box" style="grid-column: span 6;">
              <h6>Arrival</h6>
              <div class="kv one">
                <div class="row"><div class="k">Arrival Date</div><div class="v">{{ $fmt($employee->arrival_date ?? null) }}</div></div>
                <div class="row"><div class="k">Arrival Expiry</div><div class="v">{{ $fmt($employee->arrival_expiry_date ?? null) }}</div></div>
                ...
                <div class="pill-actions">
                  @if($t1)
                    <button type="button" class="btn btn-primary btn-mini view-proof" data-proof-url="{{ $t1 }}">
                      <i class="fa-solid fa-ticket"></i> Ticket
                    </button>
                  @endif
                  @if($t2)
                    <button type="button" class="btn btn-primary btn-mini view-proof" data-proof-url="{{ $t2 }}">
                      <i class="fa-solid fa-stamp"></i> Entry Stamp
                    </button>
                  @endif
                  @if($t3)
                    <button type="button" class="btn btn-primary btn-mini view-proof" data-proof-url="{{ $t3 }}">
            ...
            <div class="box" style="grid-column: span 12;">
              ...
              @if(isset($visaStages) && $visaStages->count())
                <div class="timeline">
                  @foreach($visaStages as $s)
                    @php
                    ...
                    <div class="t-item">
                      <div class="t-top">
                        <div>
                          ...
                          <div class="t-meta">
                            HR File: {{ $val($s->hr_file_number ?? null) }} · Paid: {{ $val($s->fin_paid_amount ?? null) }}
                          </div>
                        </div>
                        <div class="emp-badges">
                          <span class="badge-pill">Issue: {{ $fmt($s->hr_issue_date ?? null) }}</span>
                          <span class="badge-pill">Expiry: {{ $fmt($s->hr_expiry_date ?? null) }}</span>
                        </div>
                      </div>
                      ...
                      <div class="t-grid">
                        <div class="t-col">
                          <div class="row"><div class="k">HR Attach</div>
                            <div class="v">
                              @if($hrFile)
                                <button type="button" class="btn btn-primary btn-mini view-proof" data-proof-url="{{ $hrFile }}">View</button>
                              @else
                                N/A
                              @endif
                            </div>
                          ...
                          <div class="row"><div class="k">ICA Proof</div>
                            <div class="v">
                              @if($ica)
                                <button type="button" class="btn btn-primary btn-mini view-proof" data-proof-url="{{ $ica }}">View</button>
                              @else
                                N/A
                              @endif
                            </div>
                        ...
                        <div class="t-col">
                          <div class="row"><div class="k">Zoho Proof</div>
                            <div class="v">
                              @if($zoho)
                                <button type="button" class="btn btn-primary btn-mini view-proof" data-proof-url="{{ $zoho }}">View</button>
                              @else
                                N/A
                              @endif
                            </div>
                          ...
                          <div class="row"><div class="k">Gov Invoice</div>
                            <div class="v">
                              @if($gov)
                                <button type="button" class="btn btn-primary btn-mini view-proof" data-proof-url="{{ $gov }}">View</button>
                              @else
                                N/A
                              @endif
                            </div>
            ...
            <div class="box" style="grid-column: span 12;">
              <h6>Notes</h6>
              <div class="kv one">
                <div class="row"><div class="k">Incident Type</div><div class="v">{{ $val($employee->incident_type ?? null) }}</div></div>
                <div class="row"><div class="k">Incident Date</div><div class="v">{{ $fmt($employee->incident_date ?? null) }}</div></div>
                <div class="row"><div class="k">Remarks</div><div class="v">{{ $val($employee->remarks ?? null) }}</div></div>
                <div class="row"><div class="k">Comments</div><div class="v">{{ $val($employee->comments ?? null) }}</div></div>
              </div>
            </div>
