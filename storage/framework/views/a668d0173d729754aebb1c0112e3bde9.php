      ...
      <?php else: ?>
        <?php $__currentLoopData = $packages; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $trial): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
          <?php
            ...
            $tid=(int)($trial->trial_id ?? $trial->trials_id ?? 0);
            $cid=(int)($trial->candidate_id ?? 0);
            $clid=(int)($trial->client_id ?? 0);
          ?>
          <tr style="<?php echo e($rowStyle); ?>">
            <td><?php echo e(strtoupper($trial->trial_CN_Number ?? '')); ?></td>
            <td title="<?php echo e($candidateFull); ?>"><?php echo e($candidateDisplay); ?></td>
            <td>
              <button type="button" style="font-size:8px!important" class="btn btn-outline-primary btn-sm js-open-sales-modal" data-can-change="<?php echo e($canChange ? '1':'0'); ?>" data-candidate-id="<?php echo e($cid); ?>" data-trial-id="<?php echo e($tid); ?>" data-current-id="<?php echo e($currentSalesId); ?>" data-current-label="<?php echo e($currentSalesDisplay); ?>">
                <i class="fas fa-user-edit me-1"></i><span class="js-sales-label"><?php echo e($currentSalesDisplay); ?></span>
              </button>
            </td>
            <td class="text-center">
              <span class="badge badge-status <?php echo e($statusClass); ?>"><i class="<?php echo e($statusIcon); ?>"></i> <?php echo e($trial->trial_status); ?></span>
              <?php if($trial->trial_status==='Active' && $end): ?>
                <?php if($end->isPast()): ?>
                  <small class="time-note blink-red" title="<?php echo e($timeTitle); ?>">EXPIRED • <?php echo e($diff); ?></small>
                <?php else: ?>
                  <small class="time-note text-muted" title="<?php echo e($timeTitle); ?>"><?php echo e($diff); ?> left</small>
                <?php endif; ?>
              <?php endif; ?>
            </td>
            <td><?php echo e(strtoupper($trial->updated_at->setTimezone('Asia/Qatar')->format('d M Y h:i A'))); ?></td>
            <td><?php echo e(strtoupper($trial->nationality ?? '')); ?></td>
            <td><?php echo e(strtoupper($trial->passport_no ?? '')); ?></td>
            <td><?php echo e($partnerShort); ?></td>
            <td title="<?php echo e($clientFull); ?>"><a href="<?php echo e($clientUrl); ?>"><?php echo e($clientDisplay); ?></a></td>
            <td><?php echo e(strtoupper($trial->visa_type ?? '')); ?></td>
            <td><?php echo e($trial->arrived_date ? \Carbon\Carbon::parse($trial->arrived_date,'Asia/Qatar')->format('d M Y') : '-'); ?></td>
            <td><?php echo e($trial->display_return_date ? \Carbon\Carbon::parse($trial->display_return_date,'Asia/Qatar')->format('d M Y') : '-'); ?></td>
            <td><?php echo e($trial->pkg_cancel_date ? \Carbon\Carbon::parse($trial->pkg_cancel_date,'Asia/Qatar')->format('d M Y') : '-'); ?></td>
            <td><?php echo e($trial->pkg_expiry_date ? \Carbon\Carbon::parse($trial->pkg_expiry_date,'Asia/Qatar')->format('d M Y') : '-'); ?></td>
            <td><?php echo e($trial->trial_start_date ? \Carbon\Carbon::parse($trial->trial_start_date,'Asia/Qatar')->format('d M Y') : '-'); ?></td>
            <td><?php echo e($trial->trial_end_date ? \Carbon\Carbon::parse($trial->trial_end_date,'Asia/Qatar')->format('d M Y') : '-'); ?></td>
            <td><?php echo e($trial->remarks ?? '-'); ?></td>
            <td>
              <div class="icon-bar">
                <?php if($trial->trial_status === 'Active'): ?>
                  <a class="icon-btn" href="<?php echo e(route('packages.trialForm',['agreement_reference_no'=>$trial->agreement_reference_no])); ?>" target="_blank" data-bs-toggle="tooltip" title="Trial Form"><i class="fas fa-file-alt"></i></a>
                <?php endif; ?>
                <?php if($trial->trial_status === 'Trial Return'): ?>
                  <a class="icon-btn" href="<?php echo e(route('packages.trialReturnForm',['agreement_reference_no'=>$trial->agreement_reference_no])); ?>" target="_blank" data-bs-toggle="tooltip" title="Trial Return Form"><i class="fas fa-file-invoice"></i></a>
                <?php endif; ?>
                <button class="icon-btn js-open-status-dropdown" data-bs-toggle="tooltip" title="Change Status" data-trial-id="<?php echo e($tid); ?>"><i class="fas fa-train"></i></button>
              </div>
              <div class="dropdown-container" id="dropdownContainer-<?php echo e($tid); ?>">
                <div class="close-icon js-close-status-dropdown"><i class="fas fa-times-circle"></i></div>
                ...
                <select class="form-control status-dropdown js-status-dropdown"
                        data-trial-id="<?php echo e($tid); ?>"
                        data-candidate-id="<?php echo e($cid); ?>"
                        data-client-id="<?php echo e($clid); ?>">
                  <?php if($trial->trial_status==='Active'): ?>
...
<nav aria-label="Page navigation">
  <div class="pagination-container">
    ...
    <ul class="pagination justify-content-center"><?php echo e($packages->appends(request()->except('page'))->links('vendor.pagination.bootstrap-4')); ?></ul>
  </div>
</nav>
...
<div class="modal fade custom-modal" id="ConfirmedModal" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      ...
      <form id="ConfirmedModalForm" enctype="multipart/form-data" method="POST"><?php echo csrf_field(); ?>
        <div class="modal-body">
          ...
          <div class="row g-3">
            <div class="col-md-6">
              ...
              <input type="date" id="ConfirmedModalDate" class="form-control" name="confirmed_date" value="<?php echo e(\Carbon\Carbon::now('Asia/Qatar')->format('Y-m-d')); ?>">
            </div>
            <div class="col-md-6">
              ...
              <select id="paymentMethodForConfirmed" name="payment_method" class="form-control" required>
                ...
                <?php $__currentLoopData = $methods; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $m): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><option value="<?php echo e($m); ?>" <?php echo e(strtoupper($selected)===$m?'selected':''); ?>><?php echo e($m); ?></option><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
              </select>
            </div>
            <div class="col-md-6">
      ...
      <form method="POST" enctype="multipart/form-data" id="ChangeStatusModalForm"><?php echo csrf_field(); ?>
        <div class="modal-body">
          ...
          <div class="mb-3">
            ...
            <input type="date" id="ChangeStatusModalDate" class="form-control" name="change_status_date" value="<?php echo e(\Carbon\Carbon::now('Asia/Qatar')->format('Y-m-d')); ?>">
          </div>
          <div class="mb-3">
...
<div class="modal fade custom-modal" id="TrialReturnModal" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      ...
      <form id="TrialReturnModalForm" enctype="multipart/form-data" method="POST" novalidate><?php echo csrf_field(); ?>
        <div class="modal-body">
          <div class="row g-3 mb-3">
            <div class="col-md-6">
              <label class="form-label"><i class="fas fa-user"></i> Candidate Name</label>
...
<div class="modal fade custom-modal" id="IncidentModal" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      ...
      <form id="IncidentModalForm" method="POST" enctype="multipart/form-data"><?php echo csrf_field(); ?>
        <div class="modal-body">
          <div class="row g-3 mb-3">
            <div class="col-lg-6">
...
<script>
  (function(){
    var csrfToken=$('meta[name="csrf-token"]').attr('content')||'<?php echo e(csrf_token()); ?>';
    var routes={
      updateTrialStatus:'<?php echo e(route("packages.updateTrialStatus")); ?>',
      saveTrialConfirmed:'<?php echo e(route("packages.saveTrialConfirmed")); ?>',
      updateChangeStatus:'<?php echo e(route("packages.updateChangeStatus")); ?>',
      saveTrialReturn:'<?php echo e(route("packages.saveTrialReturn")); ?>',
      saveReturnIncident:'<?php echo e(route("package.incidentSave")); ?>',
      incidentData:'<?php echo e(route("package.incidentData",":id")); ?>',
      updateSalesName:'<?php echo e(route("packages.updateTrialSalesName")); ?>',
      refundView:'<?php echo e(route("packages.refund-view")); ?>',
      replacementView:'<?php echo e(route("packages.replacement-view")); ?>'
    };

    function openDropdown(trialId){
      if(!trialId||trialId===0){return;}
      $('.dropdown-container').hide();
      $('#fullscreenOverlay').fadeIn(120);
      $('#dropdownContainer-'+trialId).css({display:'block',opacity:0}).animate({opacity:1},160);
    }
    ...
    function ensureSpinnerKeyframes(){
      if(!document.getElementById('preloader-spin-keyframes')){
        var s=document.createElement('style');
        s.id='preloader-spin-keyframes';
    ...
    function showPreloader(){
      ...
      var h='<div id="preloader" style="position:fixed;top:0;left:0;width:100%;height:100%;background-color:rgba(255,255,255,.8);display:flex;justify-content:center;align-items:center;z-index:2000"><div style="width:42px;height:42px;border:6px solid rgba(0,0,0,.1);border-top-color:#007bff;border-radius:50%;animation:spin 1s linear infinite;margin-bottom:10px"></div><p style="font-size:12px;color:#007bff;font-weight:bold;margin:0">Loading...</p></div>';
      $('body').append(h);
    }
    function hidePreloader(){ $('#preloader').remove(); }

    function trialChangeStatus(status,tid,cid,clid){
    ...
    function updateTrialStatus(status,tid,cid,clid){
      if(!tid){ toastr.error('Missing Trial ID.'); return; }
      showPreloader();
    ...
    function fillModalFields(modalId,data,trialId){
      ...
      if(modalId==='ConfirmedModal'){
        ...
        $('#ConfirmedModalcandidateId').val(d.candidateId||'');
        $('#ConfirmedModalcandidateName').val(d.candidateName||'');
        $('#ConfirmedModalclientName').val(d.clientName||'');
        ...
        if(d.invoices&&d.invoices.length){
          $('#InvoiceData').html('<table class="table table-striped"><thead><tr><th>Date</th><th>Total Amount</th><th>Received Amount</th><th>Remaining Amount</th><th>Invoice Reference #</th></tr></thead><tbody>'+buildInvoiceRows(d.invoices)+'</tbody></table>');
        }else{
          $('#InvoiceData').html('<p class="text-danger" style="font-size:12px">No invoices found.</p>');
        }
      }
      if(modalId==='ChangeStatusModal'){
        $('#ChangeStatusModaltrial_id').val(trialId||'');
        $('#ChangeStatusModalcandidateId').val(d.candidateId||'');
        $('#ChangeStatusModalcandidateName').val(d.candidateName||'');
        $('#ChangeStatusModalclientName').val(d.clientName||'');
      }
      if(modalId==='TrialReturnModal'){
        $('#TrialReturnModaltrial_id').val(trialId||'');
        $('#TrialReturnModalcandidateId').val(d.candidateId||'');
        $('#TrialReturnModalcandidateName').val(d.candidateName||'');
        $('#TrialReturnModalclientName').val(d.clientName||'');
        $('#DecisionContent').html('');
      ...
      if(modalId==='IncidentModal'){
        $('#IncidentModaltrialId').val(trialId||'');
        $('#IncidentModalcandidateId').val(d.candidateId||'');
        $('#IncidentModalcandidateName').val(d.candidateName||'');
        $('#IncidentModalclientId').val(d.clientId||'');
        $('#IncidentModalclientName').val(d.clientName||'');
      }
    }
    ...
    function openIncidentModal(trialId,candidateId,clientId){
      $('#IncidentModaltrialId').val(trialId||'');
      $('#IncidentModalclientId').val(clientId||'');
      $('#IncidentModalcandidateId').val(candidateId||'');
      $('#IncidentModalcandidateName').val('');
      $('#IncidentModalclientName').val('');
      $('#IncidentModalpackageId').val('');
      $('#IncidentModalCN').val('');
      $('#incident_type').val('');
      $('#incident_date').val('');
      $('#IncidentRemarks').val('');
    ...
    $(document).on('click','.js-open-status-dropdown',function(){
      openDropdown($(this).data('trial-id'));
    });
    $(document).on('click','.js-close-status-dropdown',function(){
      closeAllDropdowns();
    });
    $(document).on('change','.js-status-dropdown',function(){
      var tid=parseInt($(this).data('trial-id'))||0;
      var cid=parseInt($(this).data('candidate-id'))||0;
      var clid=parseInt($(this).data('client-id'))||0;
      var status=this.value;
      if(!status){ return; }
      trialChangeStatus(status,tid,cid,clid);
    });
...
<?php /**PATH /var/www/developmentoneso-project/resources/views/package/package/partials/confirm_table.blade.php ENDPATH**/ ?>