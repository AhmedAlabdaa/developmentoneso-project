...
<div class="table-container">
    <table class="table table-striped table-hover">
        ...
        <tbody>
        <?php $__empty_1 = true; $__currentLoopData = $packages; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $candidate): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
            <?php
                $statusDateMap = [
            ...
            ?>
            <tr>
                <td><?php echo e(strtoupper($candidate->CN_Number ?? 'N/A')); ?></td>
                <td>
                  <span class="name-with-badge">
                    <?php echo e(strtoupper(optional($candidate->sales)->first_name) ?: 'N/A'); ?>

                    <span class="l-badge" aria-label="L">L</span>
                  </span>
                </td>
                <td class="text-nowrap text-center">
                  <div><?php echo e(strtoupper(\Carbon\Carbon::parse($dateToShow)->format('d-M-Y'))); ?></div>
                  <div><?php echo e(strtoupper(\Carbon\Carbon::parse($dateToShow)->format('h:i A'))); ?></div>
                </td>
                <td>
                  <?php
                  ...
                  ?>
                  <a style="color:#007bff!important" href="<?php echo e(route('candidates.show', $candidate->reference_no)); ?>" target="_blank" class="text-decoration-none text-dark" title="<?php echo e($titleName); ?>">
                    <?php echo e($displayName); ?>

                  </a>
                  <img src="<?php echo e(asset('assets/img/attach.png')); ?>" alt="Attachment" style="cursor:pointer;margin-left:8px;vertical-align:middle;height:20px" onclick="showCandidateModal('<?php echo e($candidate->candidate_name); ?>','<?php echo e($candidate->id); ?>','<?php echo e($candidate->reference_no); ?>')">
                </td>
                <td><?php echo e(strtoupper(optional($candidate->Nationality)->name ?? 'N/A')); ?></td>
                <td><?php echo e(strtoupper($candidate->passport_no ?? 'N/A')); ?></td>
                <td><?php echo e(strtoupper($candidate->labour_id_number ?? 'N/A')); ?></td>
                <td><?php echo e($candidate->passport_expiry_date ? strtoupper(\Carbon\Carbon::parse($candidate->passport_expiry_date)->format('d M Y')) : 'N/A'); ?></td>
                <td><?php echo e($candidate->date_of_birth ? strtoupper(\Carbon\Carbon::parse($candidate->date_of_birth)->format('d M Y')) : 'N/A'); ?></td>
                <td><?php echo e(strtoupper($partnerCode)); ?></td>
                <td><?php echo e(strtoupper($candidate->agreement->visa_type ?? 'N/A')); ?></td>
                <td>
                  <?php if($candidate->agreement && $candidate->agreement->client?->slug): ?>
                    <a style="color:#007bff!important" href="<?php echo e(route('crm.show', $candidate->agreement->client->slug)); ?>" target="_blank" class="text-decoration-none">
                      <?php echo e(strtoupper($candidate->agreement->CL_Number ?? 'N/A')); ?>
                ...
                <td>
                  <?php if($candidate->agreement && $candidate->agreement->client?->slug): ?>
                    <a style="color:#007bff!important" href="<?php echo e(route('crm.show', $candidate->agreement->client->slug)); ?>" target="_blank">
                      <?php echo e($sponsorName !== '' ? strtoupper($sponsorName) : 'N/A'); ?>
                ...
                <td><?php echo e($crm?->emirates_id ?? 'N/A'); ?></td>
                <td><?php echo e($candidate->agreement->created_at ? strtoupper(\Carbon\Carbon::parse($candidate->agreement->created_at)->format('d M Y')) : 'N/A'); ?></td>
                <td>
                  <?php if(isset($statusMap[$status])): ?>
                    <span class="<?php echo e($statusMap[$status]['class']); ?>">
                      <span class="st-ic"><?php echo e($statusMap[$status]['icon']); ?></span>
                      <?php echo e($statusMap[$status]['text']); ?>
                ...
                <td><?php echo e($candidate->agreement->expected_arrival_date ? strtoupper(\Carbon\Carbon::parse($candidate->agreement->expected_arrival_date)->format('d M Y')) : 'N/A'); ?></td>
                <td><?php echo e(strtoupper($candidate->agreement->vp_number ?? 'No Customer Quota')); ?></td>
                <td><?php echo e(strtoupper($candidate->agreement->selection_type ?? 'N/A')); ?></td>
                <td>
                  <?php if($candidate->agreement): ?>
                    <button type="button" class="btn btn-sm btn-outline-primary visa-status-btn" data-bs-toggle="modal" data-bs-target="#visaStatusModal" data-agreement-id="<?php echo e($candidate->agreement->id); ?>" data-vp-number="<?php echo e($candidate->agreement->vp_number ?? 'N/A'); ?>" data-current-status="<?php echo e($visaStatus ?? ''); ?>" data-update-url="<?php echo e(route('agreements.update-visa-status', $candidate->agreement->id)); ?>">
                      <?php echo e(strtoupper($visaStatus ?? 'SET STATUS')); ?>
                ...
                <td><?php echo e(strtoupper($candidate->agreement->notes ?? 'N/A')); ?></td>
                <td>
                  <?php
                    $modalKey = preg_replace('/[^A-Za-z0-9\-]/', '', (string)($candidate->CN_Number ?? 'CN'));
                  ?>
                  <?php if($subdomain === 'vienna'): ?>
                    <a href="javascript:void(0);" class="btn btn-secondary btn-icon-only ms-1" title="Transfer Candidate" data-bs-toggle="modal" data-bs-target="#transferModal-<?php echo e($modalKey); ?>">
                        <i class="fas fa-exchange-alt"></i>
                    </a>
                  <?php endif; ?>
                </td>
            </tr>
            <div class="modal fade custom-modal" id="transferModal-<?php echo e($modalKey); ?>" tabindex="-1" aria-labelledby="transferModalLabel-<?php echo e($modalKey); ?>" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title d-flex align-items-center" id="transferModalLabel-<?php echo e($modalKey); ?>"><i class="fas fa-exchange-alt me-2" style="color:#fff"></i> Transfer Candidate</h5>
                        ...
                        <div class="modal-body">
                            <div class="kv-wrap">
                                <div class="kv-grid">
                                    ...
                                    <div class="kv-value"><?php echo e(strtoupper($candidate->CN_Number)); ?></div>
                                    <div class="kv-label">Candidate Name</div>
                                    <div class="kv-value"><?php echo e($candidate->candidate_name ? strtoupper(\Illuminate\Support\Str::title(strtolower($candidate->candidate_name))) : 'N/A'); ?></div>
                                    <div class="kv-label">Passport No</div>
                                    <div class="kv-value"><?php echo e(strtoupper($candidate->passport_no)); ?></div>
                                    <div class="kv-label">Foreign Partner</div>
                                    <div class="kv-value"><?php echo e(strtoupper($candidate->foreign_partner)); ?></div>
                                </div>
                            </div>
                            <div class="mt-3">
                                <label class="form-label">Transfer to the Branch</label>
                                <select class="form-select" id="transferBranch-<?php echo e($modalKey); ?>" required>
                                    <option value="" selected>Choose Branch</option>
                                    <option value="Rozana Manpower">Rozana Manpower</option>
                                    <option value="Middleeast Manpower">Middleeast Manpower</option>
                                </select>
                                <div class="invalid-feedback" id="branchError-<?php echo e($modalKey); ?>">Please choose a branch.</div>
                            </div>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-primary transfer-save-btn" data-ref="<?php echo e($modalKey); ?>"><i class="fas fa-save me-1"></i> Transfer Save</button>
...
<script>
  ...
  function openOfficeModal(packageId){
    ...
    $('#officeForm')[0].reset();
    $.ajax({
      url:"<?php echo e(route('package.officeData',':id')); ?>".replace(':id',packageId),
      type:"GET",
  ...
  function saveOfficeData(){
    ...
    $.ajax({
      url:"<?php echo e(route('package.officeSave')); ?>",
      type:"POST",
      headers:{'X-CSRF-TOKEN':csrfToken},
      data:formData,
      processData:false,
  ...
  function openIncidentModal(packageId,cnNumber,scope){
    ...
    $.ajax({
      url:"<?php echo e(route('package.incidentData',':id')); ?>".replace(':id',packageId),
      type:"GET",
      headers:{'X-CSRF-TOKEN':csrfToken},
      dataType:"json",
      success:function(r){
  ...
  function showCandidateModal(candidateName,candidateId,referenceNo,passportNo){
    ...
    $.ajax({
      url:'<?php echo e(route('package.loadimages')); ?>',
      type:'GET',
      headers:{'X-CSRF-TOKEN':csrfToken},
      dataType:'json',
  ...
  $(document).on('click','.transfer-save-btn',function(){
        ...
        var endpoint="<?php echo e(route('package.branch-transfer-package')); ?>";
  ...
  $('#sales-save-btn').on('click',function(){
      ...
      $.ajax({
          url:'<?php echo e(route("packages.updateSalesName")); ?>',
          type:'POST',
...
<?php /**PATH /var/www/developmentoneso-project/resources/views/package/package/partials/outside_table.blade.php ENDPATH**/ ?>