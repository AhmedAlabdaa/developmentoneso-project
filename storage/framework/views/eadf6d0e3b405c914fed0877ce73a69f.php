          ...
          <table id="pr-table" class="table table-striped table-hover align-middle">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td><?php echo e(optional($r->receipt_date)->format('Y-m-d')); ?></td>
                <td><?php echo e($r->payer_type === 'customer' ? 'Customer' : 'Walk-in'); ?></td>
                <td><?php echo e($r->payer_display); ?></td>
                <td><?php echo e($r->payment_method); ?></td>
                <td class="text-end"><?php echo e(number_format((float)$r->amount,2)); ?></td>
                <td><?php echo e($r->reference_no); ?></td>
                <td>
                  <?php if(auth()->user()->role === 'Finance Officer'): ?>
                    <select class="form-select form-select-sm status-select"
                            data-id="<?php echo e($r->id); ?>"
                            data-prev="<?php echo e($r->status); ?>"
                            data-rno="<?php echo e($r->receipt_number); ?>">
                      <option value="Pending"   <?php if($r->status==='Pending'): echo 'selected'; endif; ?>>Pending</option>
                      <option value="Approved"  <?php if($r->status==='Approved'): echo 'selected'; endif; ?>>Approved</option>
                      <option value="Cancelled" <?php if($r->status==='Cancelled'): echo 'selected'; endif; ?>>Cancelled</option>
                    </select>
                  <?php else: ?>
                    <span class="badge <?php if($r->status==='Approved'): ?> bg-success <?php elseif($r->status==='Cancelled'): ?> bg-danger <?php else: ?> bg-secondary <?php endif; ?>"><?php echo e($r->status); ?></span>
                  <?php endif; ?>
                </td>
                <td><?php echo e($firstName($r->created_by)); ?></td>
                <td><?php echo e($firstName($r->approved_by)); ?></td>
                <td><?php echo e($firstName($r->cancelled_by)); ?></td>
                <td><?php echo e(optional($r->created_at)->format('Y-m-d')); ?></td>
                <td><?php echo e(optional($r->updated_at)->format('Y-m-d')); ?></td>
                <td class="text-center">
                  <button class="btn btn-secondary btn-sm view-btn"
                          data-bs-toggle="modal" data-bs-target="#prViewModal"
                          data-rno="<?php echo e($r->receipt_number); ?>"
                          data-date="<?php echo e(optional($r->receipt_date)->format('Y-m-d')); ?>"
                          data-ptype="<?php echo e($r->payer_type); ?>"
                          data-cid="<?php echo e($r->customer_id); ?>"
                          data-cname="<?php echo e($r->payer_type==='customer' ? $r->payer_display : ''); ?>"
                          data-wname="<?php echo e($r->walkin_name); ?>"
                          data-method="<?php echo e($r->payment_method); ?>"
                          data-amount="<?php echo e(number_format((float)$r->amount,2,'.','')); ?>"
                          data-ref="<?php echo e($r->reference_no); ?>"
                          data-notes="<?php echo e($r->notes); ?>"
                          data-status="<?php echo e($r->status); ?>"
                          data-attach="<?php echo e($r->attachment_path ? asset('storage/'.$r->attachment_path) : ''); ?>"
                          data-cancel="<?php echo e($r->cancel_reason); ?>">
                  ...
                  <?php if(auth()->user()->role === 'Finance Officer' && $r->status === 'Pending'): ?>
                  <button class="btn btn-danger btn-sm delete-btn" data-id="<?php echo e($r->id); ?>"><i class="fa-solid fa-trash"></i></button>
                  <?php endif; ?>
                </td>
              </tr>
...
<script>
  ...
  const storeUrl   = "<?php echo e(route('payment-receipts.store')); ?>";
  const updateUrl  = "<?php echo e(route('payment-receipts.update', ':id')); ?>";
  const destroyUrl = "<?php echo e(route('payment-receipts.destroy', ':id')); ?>";
  const statusUrl  = "<?php echo e(route('payment-receipts.status', ':id')); ?>";
  const custUrl    = "<?php echo e(route('payment-receipts.customers.search')); ?>";

  const headerImg  = <?php echo json_encode($headerImgUrl, 15, 512) ?>;
  const footerImg  = <?php echo json_encode($footerImgUrl, 15, 512) ?>;
  const nextNo     = <?php echo json_encode($nextReceiptNo, 15, 512) ?>;
  const todayISO   = <?php echo json_encode($todayISO, 15, 512) ?>;
...
<?php /**PATH /var/www/developmentoneso-project/resources/views/payment-receipts/index.blade.php ENDPATH**/ ?>