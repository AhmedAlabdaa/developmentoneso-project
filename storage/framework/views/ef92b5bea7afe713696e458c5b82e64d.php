  ...
  <table class="table table-striped table-hover">
    ...
    <tbody>
      ...
      <tr>
        ...
        <td title="<?php echo e($p->sales_name); ?>"><?php echo e(strtoupper(explode(' ', $p->sales_name)[0])); ?></td>
        <td><?php echo e(strtoupper($p->updated_at->format('d M Y h:i A'))); ?></td>
        <td><?php echo e(strtoupper($p->nationality)); ?></td>
        <td><?php echo e(strtoupper($p->foreign_partner)); ?></td>
        <td><?php echo e(strtoupper($p->CL_Number)); ?></td>
        <td><?php echo e(strtoupper($p->sponsor_name)); ?></td>
        <td><?php echo e(strtoupper($p->visa_type)); ?></td>
        <td><?php echo e(\Carbon\Carbon::parse($p->arrived_date)->format('d M Y')); ?></td>
        <td><?php echo e(\Carbon\Carbon::parse($p->returned_date)->format('d M Y')); ?></td>
        <td>-</td>
        <td><?php echo e(\Carbon\Carbon::parse($p->expiry_date)->format('d M Y')); ?></td>
        <td><?php echo e($p->description); ?></td>
        <td>
          <a href="javascript:void(0)" class="btn btn-primary btn-icon-only" title="Change Status" onclick="openStatusDialog('<?php echo e($p->id); ?>','<?php echo e($p->candidate_name ? strtoupper(\Illuminate\Support\Str::title(strtolower($p->candidate_name))) : 'N/A'); ?>')">
            <i class="fas fa-train"></i>
          </a>
          <a href="<?php echo e(route('package.exit', ['reference_no' => $p->hr_ref_no])); ?>" class="btn btn-primary btn-icon-only" title="Exit Form" target="_blank">
            <i class="fas fa-file-export"></i>
          </a>
          <?php if(Auth::user()->role === 'Admin'): ?>
          <form action="<?php echo e(route('packages.destroy', $p->id)); ?>" method="POST" style="display:inline" id="delete-form-<?php echo e($p->id); ?>">
            <?php echo csrf_field(); ?>
            <?php echo method_field('DELETE'); ?>
            <button type="button" class="btn btn-danger btn-icon-only" onclick="confirmDelete('<?php echo e($p->id); ?>')" title="Delete">
...
<script>
  ...
  function openIncidentModal(packageId){
    ...
    $.ajax({
      url:"<?php echo e(route('packages.update-status-inside', ':id')); ?>".replace(':id',packageId),
      type:'GET',
...
<?php /**PATH /var/www/developmentoneso-project/resources/views/package/partials/incident_table.blade.php ENDPATH**/ ?>